...
## 2026-08-04：短線第 30 輪 headline 首次勝 QQQ，但成本與尾部反轉；13/20 不升格

- 在任何新 overlay 結果前，先固定第 29 輪收據、同一 905 宗事件、五個 20% 槽、非事件
  QQQ 底倉、Top-7／eligible／complete／QQQ placebo 四條疊加、三條買入並持有及第 29 輪
  現金路徑。正常事件收取 QQQ sell、basket buy、basket sell、QQQ buy 四腿；每資產
  20／50／100 bp、七假說 family、NW lag 20、63-session／20,000 路徑共同 bootstrap、
  固定半期、2008／2020／2022、最佳三年及 46-event 尾部全部事前固定。協議 SHA-256
  `d17979ca149c20341cc537290743ee44944ce0a6952ea0409995e77e85fcd601`，先以 commit
  `bf52098` 推送，再首次計算。
- 每資產 20 bp 下，Top-7／QQQ 替換式疊加由 US$1,000 至 **US$27,067**、CAGR
  **17.94%**、SHY 超額 Sharpe **0.74**、最大跌幅 **-52.48%**；QQQ 買入並持有為
  US$21,797、16.67%、0.71、-53.44%。這是同一已見資金路徑中首次 headline 高於 QQQ。
- 但候選相對 QQQ 平均日差的 NW t 只有 **0.66**，Holm／共同 max-t p 為
  **0.5117／0.9368**，6,221 次 Bonferroni p 為 1.00；對完整現時股池 overlay 的 t 亦只有
  **1.57**。前半平均差為負，只有後半較強，故固定半期門檻失敗。
- 移除相對 QQQ 最佳三年 2026／2025／2016 後，平均日差轉負，NW t **-0.97**。移除事前
  定義最有利 46 宗事件後，候選 CAGR 落後 QQQ **3.76pp**，並落後完整股池 overlay
  **1.07pp**。headline 依賴時間及事件右尾。
- 候選年率化換手 **36.3 倍**。每資產成本升至 50／100 bp 時，候選 CAGR 降至
  **11.68%／1.97%**，分別落後 QQQ **4.98／14.70pp**；正常事件四腿的名義總成本相當於
  100／200 bp。2008／2020 候選亦落後 QQQ，只有 2022 較佳，危機門檻不通過。
- 二十項事前門檻只過 **13/20**；29/29 控制及 29/29 單欄變異攻擊拒收只證明凍結協議
  正確執行。決定：`not_rejected_by_round30=false`、`new_strategy_created=false`；不因
  US$27,067 headline 建立策略或調低成本。正式就緒 1/18、point-in-time 1/20、合資格
  provider package 0、正式 run 0、Paper 全現金、持倉 0、實金 US$0。

## 2026-08-04：短線第 29 輪完成五槽資金會計；13/18 不升格

- 在任何日曆路徑結果前，先固定第 28／27／24 輪及原始事件收據、905 宗事件、25 隻現時
  survivor cohort、五個 20% 資金槽、`event_index mod 5` assignment、D+1 open 至第 20 個
  session close、20／50／100 bp 成本、七路徑、六基準 family、NW lag 20、63-session／
  20,000 路徑共同 bootstrap、固定前後半、2008／2020／2022 及移除最佳三年。協議
  SHA-256 為 `020f99c561994cf59b5d196b17c20aaa3c2fc2bd925ac1711384fcbcfd378b25`，先以
  commit `65b3789` 推送，再首次計算。
- 五槽各有 181 宗事件，同槽無重疊，最高同時使用五槽；5,028 個交易日的槽位、現金、成本、
  總資產及無槓桿 identity 全部重播。20 bp 下 Top-7 由 US$1,000 至 **US$10,189**，CAGR
  **12.31%**、SHY 超額 Sharpe **0.67**、最大跌幅 **-38.74%**、平均持倉 72.0%、年率化
  換手 18.1 倍。
- QQQ 買入並持有同期至 **US$21,797**、CAGR **16.67%**；SPY CAGR 11.26%。Top-7 對
  合資格池 NW t 2.70，Holm／共同 max-t p 0.0343／0.0301；但對完整現時股池 t 只有
  **1.51**，對 SPY t 只有 **0.13**，對 QQQ 買入並持有的日均差為負。全專案 6,214 次
  Bonferroni p 為 1.00。
- 前後半一致性失敗；移除最佳三年 2026／2009／2025 後，對合資格池 NW t 只有 **1.55**。
  2008／2022 候選回報為 **-33.29%／-25.72%**；100 bp 下 CAGR 降至 **4.52%**。候選在
  成本壓力下仍高於三個事件式基準，只代表相對排名沒有反轉，不修復 survivor、退市、容量、
  稅項或零碎股成交缺口。
- 十八項事前門檻只過 **13/18**；25/25 控制及 25/25 單欄變異攻擊拒收只證明凍結協議
  正確執行。決定：不事後調 Top-K、持有期、槽數或市況濾網；正式就緒 1/18、point-in-time
  1/20、合資格資料包 0、策略 run 0、Paper 全現金、持倉 0、實金 US$0。

## 2026-08-04：短線第 28 輪控制後 top-middle 只餘 42%／35%；6/14 不升格

- 在任何歸因統計前先固定第 27 輪 905 個事件與 bucket、訊號日前 5 日回報、20 日已實現
  波幅、平均 rank、兩控制橫截面 OLS、八假說 Holm／共同 max-t、訊號日 QQQ 20 日市況及
  46-event 尾部。協議 SHA-256 為
  `3316ca8acdd46a80d591ca6d7a810d8f820ce1073052eb35d5e8aea24fc724c0`，先以 commit
  `2ebb973` 保存，再首次計算；沒有搜尋其他控制窗或刪除事件。
- 原始 top-middle 完全重播第 27 輪：eligible／complete 平均 +0.516／+0.400 個百分點，
  NW t 2.57／2.30。控制 5 日回報與 20 日波幅後，殘差只餘 +0.216／+0.140 個百分點，
  NW t **1.62／1.10**，平均保留率只有 **41.8%／34.9%**；complete 後半平均轉為
  **-0.068 個百分點**。
- raw bottom-middle +0.145／+0.149 個百分點；控制預測 -0.019／+0.096，residual 仍為
  **+0.164／+0.053 個百分點**。bottom 的 5 日 rank 較低，但 5 日 beta t 只有 -1.41／
  -0.28；complete 波幅 beta t 2.81，平均波幅貢獻 t 仍不足 1.96。不能把底段反彈簡化為
  單一短期反轉或高波幅補償。
- 訊號日 QQQ 20 日為負的 228 個事件，eligible／complete residual top-middle 平均皆負；
  移除 raw bottom-middle 最大 46 個事件後 NW t 只有 **1.95／1.01**。eligible 的 1.9506
  嚴格低於事前 1.96，不以四捨五入升格。
- 十四項反證只過 **6/14**；23/23 控制與 23/23 單欄變異攻擊拒收只證明協議執行正確。
  決定：不改 2／10 日反轉窗、不改 10／60 日波幅、不只選 QQQ 強市；正式就緒 1/18、
  point-in-time 1/20、策略 run 0、Paper 全現金、持倉 0、實金 US$0。
- 首次 Linux 每日重建只發現逐事件 feature hash 的 BLAS 最末位漂移，所有呈列統計與門檻
  均不變；因此把 hash 輸入明確量化至 8 位小數，另保留完整收據 12 位小數與 `1e-12`
  歸因 identity 門檻。這是跨平台收據修復，不是更改模型、樣本或結果。
...
- 首次 Pages Linux 重建顯示理論零殘差的正負號會因數值庫而異；修復只把絕對值不高於
  `1e-12` 的值在「正值比例」分類為零，並把收據負零正規化。平均、NW t、Holm、共同
  max-t、14 道門檻及決策全數不變；新增近零跨平台回歸測試後才重新發佈。

## 2026-08-04：短線第 25 輪名義 Top-7 中位只得 2.21 個有效注數；7/12 不升格

- 在重建逐股 Top-7、相關系數及現時代號貢獻前，先固定三個台股參考 commit、行情／
  watchlist／事件收據 hash、25 隻現時 cohort、905 個 20 日事件、60 日 Pearson 相關、
  `corr > 0.70`、cap 2、不回補、有效注數公式、四假說 Holm 及 52-event／20,000 路徑
  共同 max-t。父協議 SHA-256 為
  `f4510c4a3166a8c7f742224a17785e505f2c9214a13977ea426b6c4fecb54db7`，先以 commit
  `00faaa5ed04bccb8b0d147fb47a9ef3c706f3d44` 保存。
- 父協議首次運行在刪除主要現時代號後遇到合資格股份不足 7 隻，以
  `crowding_baseline_fairness_breached` 停止，沒有寫出結果。其後先提交唯一
  [matched-cash repair](SHORT_TERM_CORRELATION_CROWDING_SCHEMA_REPAIR_PROTOCOL.md)：不足
  7 隻時每隻仍佔 1/7，餘額留現金，candidate 與 baseline 使用完全相同股票持倉及成本。
  修補 SHA-256 為 `8f7c3428cb3fb9b7c4ba95bf579f82b712e7a5865474a8b7991c013d6f949590`，
  commit `c48de060cbc501f04e73fa9977d040eff8fc097d`；最終結果明示為非獨立證據。
- 原 Top-7 平均 pairwise correlation 0.377，中位有效獨立注數只有 **2.21/7**；
  **86.0%** 事件低於 3 注，71.9% 至少有一對相關高於 0.70。名義持有七隻，風險上並非
  七個獨立押注。
- 選中頻率沒有被單一代號壟斷：最高單一／前三 slot share 只有 5.7%／16.1%。25 條
  leave-one-symbol-out 全部平均為正，最低 NW t 2.38；但刪除事後淨貢獻最高三個現時
  代號後平均差只餘 +0.120 個百分點、NW t **1.50**、Holm p 0.1341、共同 max-t p
  0.2372，顯示組合依賴而非單一股依賴。代號只是 2026 現時 ticker，不是永久 ID 或
  買入名單。
- 固定 cap-2／不回補壓力保留 94.9% 平均股票持倉，配對差仍為 +0.311 個百分點、NW t
  3.06；但平均相關只由 0.377 降至 0.369，即 **0.008**，未達事前 0.05，不能把台股
  GUARD 風險規則直接移植成美股新策略。十二項反證只過 **7/12**；19/19 控制及 19/19
  攻擊拒收只證明協議執行正確。
- 決定：不搜尋 cap 1、其他相關門檻、40／80 日窗口、回補或重新配重救援。正式 v1、
  6,208-trial DSR 及 Paper 門檻不變；正式就緒 1/18、策略 run 0、Paper 全現金、持倉 0、
  實金 US$0。下一個可升級證據仍是獲授權 point-in-time／永久 ID／退市數據。
