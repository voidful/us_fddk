# 短線成交量突破 Top-10 分散度單一帳戶資金診斷協議（Round 62）

版本：1.0（post-hoc concentration-stress diagnostic）  
固定日期：2026-08-10

## 研究角色

Round 61 的固定成交量確認突破 Top-5 資金曲線出現集中度風險。本輪只把同一訊號、同一
快照、同一成本及同一 non-overlap 會計改為固定 Top-10，量度分散持倉對回撤及 ETF 基準
差距的影響。這是看到 Round 61 後新增的 post-hoc 風險診斷，不是獨立首次 alpha 證據；
結果不建立策略、Paper 或實金行動。
...
- 按 20 日回報由高至低取 **Top-10**；不足 1 檔即跳過；不套用市場方向濾網。
...
本輪是單一固定 Top-10 concentration-stress family，global trial lower bound 由 6313
增加至少 1 至 6314；不聲稱精確增量。研究角色固定為 `concentration_stress_diagnostic`。
