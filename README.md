...
- 短線第十八輪在正式結果出現前凍結[一次性正式回測事前登記](docs/SHORT_TERM_FORMAL_BACKTEST_PREREGISTRATION.md)，補上既有 QQQ／SPY execution package 沒有的同步 US 1M T-bill 日回報，並把「同股漂移」明確改成首個正式訊號 Top-10 只買一次後漂移。四個 baseline、US$1,000、raw open、10／25／50 bps、公司行動單次入賬、當時 6,208 trials DSR 及四路十段 PBO 全部事前固定；合成就緒控制 **18/18**，RF 缺日／單位、run ID、baseline、成本、統計及決策邊界攻擊 **18/18 拒收**。原始 6,208 收據不改，但正式政策已另綁定[不可回減全域試驗帳本](docs/SHORT_TERM_GLOBAL_TRIAL_LEDGER_PROTOCOL.md)；獨立審閱補記第 24–28 輪已查看的 39 個共同假說後，已見及預留 family 的保守下限提高至 **6,287**。真實正式就緒仍為 **1/18**、provider package 0、RF 包 0、策略運行 0；完整[就緒報告](docs/SHORT_TERM_FORMAL_BACKTEST_READINESS_REPORT.md)保留，Paper 全現金、實金 US$0。
...
- 短線第二十七輪先凍結[排序單調性與隨機 placebo 協議](docs/SHORT_TERM_RANK_MONOTONICITY_PLACEBO_PROTOCOL.md)，把同一 905 個事件的 eligible 與完整現時 25 股股池各自按 20 日動量切成三段，固定八假說 family、20 組隨機排序 placebo、QQQ 升跌市及移除 46 個最大差額事件。高段對中段有局部差額：eligible／complete NW t **2.57／2.30**；但中段對低段平均反為 **-0.145／-0.149 個百分點**，top-bottom t 只有 **1.46／1.09**，完整股池 t 亦低於最強 placebo **1.22**。QQQ 下跌組兩個平均皆負，尾部壓力 t 只有 **1.46／1.93**，十四項反證只過 **5/14**。控制 **23/23**、突變攻擊 **23/23 拒收**；完整[研究報告](docs/SHORT_TERM_RANK_MONOTONICITY_PLACEBO_RESEARCH_REPORT.md)保留，沒有事後只選 top-middle 建策略，正式就緒 1/18、策略 run 0、Paper 全現金、實金 US$0。
- 短線第三十九輪在任何結構選擇或回報結果前凍結[龍頭回調—回升 10 日協議](docs/SHORT_TERM_LEADER_PULLBACK_REBOUND_PROTOCOL.md)，只在第 38 輪同一 905 宗已見 survivor 事件的原 Top-7 內，事前固定 3%–16% 回調、收市回升、ATR14 及 reward/risk 不低於 1.60；D+1 調整開市買入，第 10 個 session 調整收市沽出，未入選部分持有 QQQ。只有 **151/905** 宗事件非空，平均候選 0.24 隻；10 bp 單向成本下，US$1,000 候選期末 **US$21,479**、CAGR **16.58%**，低於 QQQ 的 **US$21,756／16.66%**，亦低於相同比例原 Top-N 的 **US$21,674／16.64%**。相對 QQQ／Top-N 的 NW t 分別為 **-0.41／-0.31**，共同 max-t p **0.9970／0.9994**；25／50 bps、US$0.01／US$0.05 子委託費、最佳三年及 46 宗事件移除均完整保留。協議把 2006-08-07–2026-07-31 寫成 5,028 個比較交易日，但實際只有 **5,027**；加 2006-08-04 一列成交前現金才是 5,028，所以 `protocol_calendar_internal_consistency=false`，沒有改寫協議救援。二十二項只過 **8/22**，控制 **75/76**、突變攻擊 **72/72 拒收**；九路期末均全現金、零持倉。完整[研究報告](docs/SHORT_TERM_LEADER_PULLBACK_REBOUND_RESEARCH_REPORT.md)保留，不呈列最新逐股或逐筆委託，短線 Paper 全現金、實金 US$0。
- 美國議員與企業內部人公開披露 Phase 1 只建立[數據就緒報告](docs/SHORT_TERM_DISCLOSURE_READINESS_REPORT.md)：事前固定眾議院／參議院 PTR、SEC Form 4、Schedule 13D／13G 及 Form 13F 六種資料類型，但必須分開事件日、公開可得日與策略已知日。Congress PTR 有延遲、金額區間及配偶／受養人資訊，Form 4、13D／13G 與 13F 也各有不同經濟語意，不得當成即時、精確成交帶。Phase 1 實際 readiness 只過 **2/20**；Congress 商業用途禁限尚未有書面法律／授權判定，因此不擷取、不選股，動態選擇停用，策略運行 0、Paper Trading（模擬交易）全現金、實金 US$0。公開輸出不列人名、股票代號、原始列或逐文件賬本。
- 短線第四十一輪 Form 4 candidate specification 已在任何真實資料或績效前 result-blind 凍結（不是已啟用的動態選股策略）：[多內部人申報購買 × 公開後量價確認協議](docs/SHORT_TERM_FORM4_CLUSTER_PROTOCOL.md)及[v1.1 修訂](docs/SHORT_TERM_FORM4_CLUSTER_PROTOCOL_AMENDMENT_V1_1.md)把 `P` 誠實保留為「公開市場或私人購買」，cluster 必須是兩組已驗證獨立經濟資本；4/A、共同持有、失敗 cluster、cooldown、PIT 配對及 20 年 filing／known-at 分母均有固定 fail-closed 規則。候選仍是八個比較，但專屬 admission 現為 **0/16**、20 年 known-at 未驗證、真實事件 0、策略運行 0；Congress PTR 不混入，沒有最新標的、Paper 或實金動作。
- 短線第四十二輪按[事前協議](docs/SHORT_TERM_FORM4_ADMISSION_FEASIBILITY_PROTOCOL.md)及[schema v1.1 修訂](docs/SHORT_TERM_FORM4_ADMISSION_FEASIBILITY_SCHEMA_AMENDMENT_V1_1.md)固定處理 2006Q1／2016Q3／2026Q2 共 **12 份** Form 4／4-A 細樣本。收集程序完成 5 個 HTTP request 後遇到 1 次 post-fetch 本地驗證失敗；complete-submission request 0、cold replay 未完成，故准入只為 **2/16**，並以 `form4_feasibility_daily_index_missing_or_ambiguous` 停止。完整[准入可行性報告](docs/SHORT_TERM_FORM4_ADMISSION_FEASIBILITY_REPORT.md)只公開總樣本數、門檻、停止碼及雜湊綁定收據，不列人物、CIK、accession、公司、股票代號、文件位置或回報數值；候選選擇 0、策略運行 0、Paper 全現金、實金 US$0，今天不下單。
- 短線第四十三輪的原始[Form 4 多路徑前瞻准入協議 v1.0](docs/SHORT_TERM_FORM4_MULTIPATH_FORWARD_PROTOCOL.md)經獨立審核後保留為不可執行的失敗紀錄；有效的[append-only v1.1 修訂](docs/SHORT_TERM_FORM4_MULTIPATH_FORWARD_PROTOCOL_AMENDMENT_V1_1.md)及其[機器收據](artifacts/short_term_form4_multipath_forward_protocol_amendment_v1_1_receipt.json)只准 synthetic structural 資料工程，撤銷 504／100／50及所有績效解封；目前不具真實 SEC response／known-at 外部信任根，故不構成 real evidence admission。新版離線核對把同一 accession 的 1–10 條自洽 SEC archive path 視為專案級等價類（不是人物數或 SEC 官方上限），強制完整 `d0`／首個較後 `d1`、原始 path、完整 submission、create-once first-observed registry及父證據鏈；[人物披露動態選股設計](docs/SHORT_TERM_ACTOR_DISCLOSURE_DYNAMIC_SELECTION_DESIGN.md)仍只屬設計。Congress PTR 法律／exact-use 未清，收集 0；SEC Form 4 仍只為 **2/16**，本輪 request 0、候選 0、配置 0、績效 0、Paper 全現金、實金 US$0，今天不下單。任何未來策略都要另開新 Round、新 trial family及全新未見前瞻 cohort。
- 短線第五十四輪凍結[成交量確認突破診斷協議](docs/SHORT_TERM_VOLUME_BREAKOUT_DIAGNOSTIC_PROTOCOL.md)：每週完成 XNYS 收市後，以 60 日收市突破、1.5 倍 20 日中位成交量及 Top-5 20 日動量做 5／10／20 日事件比較。403 宗完整事件的 20 日配對差 **+0.753 個百分點**、NW t **2.03**、bootstrap 下界 **+0.092 個百分點**，但勝率 **49.9%**，只過 **5/6** gate；結果屬現時股池 survivorship-biased near miss，不建立策略、Paper 或個股名單，完整[診斷報告](docs/SHORT_TERM_VOLUME_BREAKOUT_DIAGNOSTIC_REPORT.md)與不可回減收據保留，今天不下單。
- 短線第三十八輪在任何選擇或回報結果前凍結[四窗動量共振協議](docs/SHORT_TERM_MULTI_WINDOW_RESONANCE_PROTOCOL.md)，沿用同一 905 宗事件、五槽、D+1、20 日持有期及 QQQ 底倉；5／10／15／20 日各取 Top-7，至少三窗入選才佔一個七分之一子槽，未填部分留在 QQQ。每資產 20 bp 下，候選由 US$1,000 至 **US$22,654**、CAGR **16.89%**，只略高於 QQQ 的 **US$21,797／16.67%**，卻落後第 30 輪原 Top-7 的 **US$27,067／17.94%** 及相同比例 20 日排名的 **US$26,632／17.84%**。相對 QQQ NW t 只有 **0.19**、Holm／max-t p **0.8519／1.0000**；移除 2026／2025／2016 後 t **-1.43**，50／100 bp 時落後 QQQ **4.81／12.72 個百分點** CAGR，移除最有利 46 宗事件後亦落後 QQQ **3.97 個百分點**。二十項只過 **11/20**，控制 **45/45**、突變攻擊 **39/39 拒收**；完整[研究報告](docs/SHORT_TERM_MULTI_WINDOW_RESONANCE_RESEARCH_REPORT.md)保留，複雜度沒有勝過簡單規則，不呈列最新逐股名單，point-in-time 就緒 1/20、正式策略 run 0、Paper 全現金、實金 US$0。
- 短線第三十輪在任何新路徑結果前凍結[QQQ 全投資替換式疊加協議](docs/SHORT_TERM_QQQ_REPLACEMENT_OVERLAY_PROTOCOL.md)，保留同一 905 宗事件、五槽、Top-7、D+1 及 20 日持有期，只把非事件現金改為 QQQ；正常事件完整收取 QQQ 沽出、股票買入、股票沽出及 QQQ 買回四腿成本。每資產 20 bp 下，候選由 US$1,000 至 **US$27,067**、CAGR **17.94%**，headline 首次高於 QQQ 的 **US$21,797／16.67%**；但相對 QQQ NW t 只有 **0.66**，Holm／max-t p **0.5117／0.9368**，移除 2026／2025／2016 後 t **-0.97**。每資產 50／100 bp 時分別落後 QQQ 4.98／14.70 個百分點 CAGR，移除最有利 46 宗事件亦落後 QQQ 3.76 個百分點；二十項只過 **13/20**。控制 **29/29**、突變攻擊 **29/29 拒收**；完整[研究報告](docs/SHORT_TERM_QQQ_REPLACEMENT_OVERLAY_RESEARCH_REPORT.md)保留，沒有建立策略，正式就緒 1/18、策略 run 0、Paper 全現金、實金 US$0。
- 短線第二十九輪在任何資金路徑結果前凍結[日曆時間資金佔用協議](docs/SHORT_TERM_CALENDAR_CAPITAL_ACCOUNTING_PROTOCOL.md)，把同一 905 個重疊事件按次序放進五個 20% 資金槽，D+1 open 至第 20 個交易日 close、不借貸，並固定 20／50／100 bp 成本、七條同起訖日路徑、六基準共同 family、危機年及移除最佳三年。20 bp 下 Top-7 由 US$1,000 至 **US$10,189**、CAGR **12.3%**、SHY 超額 Sharpe 0.67、最大跌幅 -38.7%；但 QQQ 買入並持有至 **US$21,797**、CAGR **16.7%**。對完整現時股池 NW t 只有 **1.51**，移除最佳三年只餘 **1.55**，6,214 次 Bonferroni 不通過；十八項只過 **13/18**。控制 **25/25**、突變攻擊 **25/25 拒收**；完整[研究報告](docs/SHORT_TERM_CALENDAR_CAPITAL_ACCOUNTING_RESEARCH_REPORT.md)保留，正式就緒 1/18、策略 run 0、Paper 全現金、實金 US$0。
- 短線第二十八輪在任何新歸因前凍結[短期反轉與波幅歸因協議](docs/SHORT_TERM_REVERSAL_VOLATILITY_ATTRIBUTION_PROTOCOL.md)，沿用第 27 輪全部 905 個 bucket，只以訊號日前 5 日回報 rank 及 20 日已實現波幅 rank 做每事件橫截面 OLS。控制後 eligible／complete top-middle 平均只保留 **41.8%／34.9%**，NW t 由 **2.57／2.30** 降至 **1.62／1.10**；完整股池後半更轉為 -0.068 個百分點。residual bottom-middle 仍為正，訊號日 QQQ 20 日為負時兩個 residual top-middle 亦為負；尾部 t 只有 **1.95／1.01**，十四項反證只過 **6/14**。控制 **23/23**、突變攻擊 **23/23 拒收**；完整[研究報告](docs/SHORT_TERM_REVERSAL_VOLATILITY_ATTRIBUTION_RESEARCH_REPORT.md)保留，沒有事後改窗口或只選強市，正式就緒 1/18、策略 run 0、Paper 全現金、實金 US$0。
...
# 重建第十八輪正式回測就緒稽核：原始 6,208、現行下限 6,287 trials、十八道控制／攻擊
...
# 重建第三十八輪四窗動量共振：略高於 QQQ，但落後原 Top-7 與同曝險 20 日排名
# 11/20 不升格；不輸出最新逐股交易名單，Paper 仍全現金
python scripts/build_short_term_multi_window_resonance_report.py

# 重建第三十輪 QQQ 全投資替換式疊加：20 bp headline 高於 QQQ，但 13/20 不升格
# 50／100 bp、固定半期、最佳年份及 46-event 尾部均保留；Paper 仍全現金
python scripts/build_short_term_qqq_replacement_overlay_report.py
