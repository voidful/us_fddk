...
- 短線第四十三輪以全新[Form 4 多路徑前瞻准入協議](docs/SHORT_TERM_FORM4_MULTIPATH_FORWARD_PROTOCOL.md)處理 Round42 暴露的工程假設，但不改寫或重啟舊失敗：同一 accession 可有 1–10 條自洽 SEC index path，離線 resolver 固定用 `(10 位 CIK, path)` 決定 canonical，並要求只在 `d0`／下一 index 日 `d1` 其中之一出現。新的[人物披露動態選股設計](docs/SHORT_TERM_ACTOR_DISCLOSURE_DYNAMIC_SELECTION_DESIGN.md)把 Congress PTR 與 SEC 法定角色分流，固定 PIT 股份池、單股單一狀態、QQQ／SPY／同池及配對負控制；但 Congress 法律用途未清、Form 4 仍只為 **2/16**，故本輪 request 0、候選 0、績效 0。前瞻績效只可在固定第 504 個新增 XNYS session 且至少 100 次配置／50 間發行人時一次解封，否則以統計力不足停止；Paper 全現金、實金 US$0，今天不下單。
