...
test.skip("legacy detailed report is retained as an internal research log", async () => {
  ...
  assert.match(html, /今天不下單；11\/20 不是候選 Paper 的啟動條件/);
  ...
  assert.match(html, /Headline、完整股池、QQQ及多重檢驗一次呈列/);
  assert.match(html, /高換手令 20 bp 優勢在較高成本及尾部壓力下反轉/);
  assert.match(html, /二十九道輸入、QQQ 底倉、四腿成本、統計及決策控制/);
  assert.match(html, /二十九項 hash、底倉、成本、family、尾部及越權偷換全拒收/);
  ...
  assert.match(html, /刪除最佳年份及最大正事件，集中度完整披露/);
  assert.match(html, /46 列佔全部正配對差 (?:<!-- -->)?30\.5%/);
  assert.match(html, /1% 與 5% 對稱截尾均保留/);
  assert.match(html, /八項門檻逐項呈列；7\/8 不升格/);
  assert.match(html, /十五道輸入、時間、尾部、重抽及決策邊界控制/);
  assert.match(html, /十五項路徑、時間、尾部、bootstrap 及越權偷換全拒收/);
  ...
  assert.match(html, /2\.76%/);
  assert.match(html, /十二道輸入、格網、亂數、公平基準及統計控制/);
  assert.match(html, /十二項期限、Top-K、壓力格、亂數及 baseline 偷換全拒收/);
...
test.skip("legacy detailed investment report is retained as an internal research log", async () => {
  const response = await render();
  assert.equal(response.status, 200);
  assert.match(response.headers.get("content-type") ?? "", /^text\/html\b/i);
  const html = await response.text();
