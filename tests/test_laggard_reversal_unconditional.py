...
from usfddk.global_trial_ledger_round59_extension import audit_round59_extension
from usfddk.short_term_laggard_reversal_unconditional import (
...
def test_round59_unconditional_protocol_is_frozen_and_self_bound() -> None:
    ...
    assert loaded["status"] == "posthoc_laggard_reversal_unconditional_diagnostic_only"
    assert loaded["research_role"] == "posthoc_robustness_diagnostic"
    assert loaded["independent_first_seen_evidence"] is False
    assert loaded["strategy_rule_changed"] is False
    assert loaded["control_definition"]["market_filter"] == "none"
    assert loaded["paper_authorized"] is False
    assert loaded["real_money_authorized"] is False
    assert loaded["today_action"] == "今天不下單"


def test_round59_unconditional_is_posthoc_control_and_trade_blind() -> None:
    ...
    assert validation["research_role"] == "posthoc_robustness_diagnostic"
    assert validation["complete_event_counts"] == {"5": 755, "10": 751, "20": 747}
    ...
    assert validation["multiplicity"]["current_lower_bound"] == 6311
...
def test_round59_trial_ledger_extension_is_append_only() -> None:
    result = audit_round59_extension(root=ROOT)
    assert result["passed"] is True
    assert result["current_lower_bound"] == 6311
