...
from usfddk.global_trial_ledger_round56_extension import audit_round56_extension
from usfddk.short_term_gap_reversal_diagnostic import (
...
def test_round56_protocol_is_frozen_and_self_bound() -> None:
...
def test_round56_validation_is_negative_and_trade_blind() -> None:
    ...
    unsigned.pop("receipt_sha256")
    assert _canonical_sha256(unsigned) == claimed
    assert validation["status"] == "gap_reversal_diagnostic_negative_survivorship_biased"
    assert validation["complete_event_counts"] == {"5": 48, "10": 48, "20": 48}
    ...
    assert validation["multiplicity"]["current_lower_bound"] == 6302
...
def test_round56_trial_ledger_extension_is_append_only() -> None:
    result = audit_round56_extension(root=ROOT)
    assert result["passed"] is True
    assert result["current_lower_bound"] == 6302
