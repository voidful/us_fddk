...
from usfddk.global_trial_ledger_round54_extension import audit_round54_extension
from usfddk.short_term_volume_breakout_diagnostic import (
...
def test_round54_protocol_is_frozen_and_self_bound() -> None:
    ...
    assert _load_protocol(ROOT)["status"] == "preregistered_volume_breakout_diagnostic_only"
...
def test_round54_validation_is_near_miss_and_trade_blind() -> None:
    ...
    unsigned.pop("receipt_sha256")
    assert _canonical_sha256(unsigned) == claimed
    assert validation["status"] == "volume_breakout_diagnostic_negative_survivorship_biased"
    assert validation["complete_event_counts"] == {"5": 403, "10": 403, "20": 403}
    ...
    assert validation["multiplicity"]["current_lower_bound"] == 6296
...
def test_round54_trial_ledger_extension_is_append_only() -> None:
    result = audit_round54_extension(root=ROOT)
    assert result["passed"] is True
    assert result["current_lower_bound"] == 6296
