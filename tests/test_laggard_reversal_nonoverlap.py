...
from usfddk.global_trial_ledger_round60_extension import audit_round60_extension
from usfddk.short_term_laggard_reversal_nonoverlap import (
...
def test_round60_nonoverlap_protocol_is_frozen_and_self_bound() -> None:
    ...
    assert loaded["status"] == "posthoc_laggard_reversal_nonoverlap_capital_diagnostic_only"
...
def test_round60_nonoverlap_capital_curve_fails_gates_and_is_trade_blind() -> None:
    ...
    unsigned.pop("receipt_sha256")
    assert _canonical_sha256(unsigned) == claimed
    assert validation["status"] == "laggard_reversal_nonoverlap_capital_negative_survivorship_biased"
    assert validation["capital_policy"] == {
        "accepted_events": 123,
        "candidate_events": 747,
        "holding_sessions": 20,
        "ignored_overlapping_events": 624,
        "overlap_policy": "ignore_signals_until_exit_then_resume",
        "starting_capital_usd": 1000.0,
    }
    assert validation["gate_summary"]["passed"] == 3
    assert validation["gate_summary"]["required"] == 7
    selected = validation["scheduled_baselines"]["selected"]
    assert selected["cagr"] == 0.09141942256962698
    assert selected["max_drawdown"] == -0.6260958675022703
    assert validation["passive_baselines"]["SPY"]["cagr"] > selected["cagr"]
    ...
    assert validation["multiplicity"]["current_lower_bound"] == 6312
...
def test_round60_trial_ledger_extension_is_append_only() -> None:
    result = audit_round60_extension(root=ROOT)
    assert result["passed"] is True
    assert result["current_lower_bound"] == 6312
