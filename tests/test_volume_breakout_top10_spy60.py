...
from usfddk.global_trial_ledger_round63_extension import audit_round63_extension
...
def test_round63_protocol_is_frozen_and_marks_regime_overlay() -> None:
    ...
    assert loaded["status"] == "posthoc_volume_breakout_top10_spy60_regime_diagnostic_only"
    assert loaded["research_role"] == "market_regime_overlay_diagnostic"
    ...
    assert loaded["signal"]["market_filter"] == "SPY_close_above_60_session_SMA"
    assert loaded["paper_authorized"] is False
    assert loaded["real_money_authorized"] is False
    assert loaded["today_action"] == "今天不下單"


def test_round63_regime_curve_passes_capital_gates_but_stays_paper_blind() -> None:
    ...
    unsigned.pop("receipt_sha256")
    assert _canonical_sha256(unsigned) == claimed
    assert validation["status"] == "volume_breakout_top10_spy60_regime_positive_survivorship_biased"
    ...
    assert validation["gate_summary"]["passed"] == 7
    assert validation["gate_summary"]["required"] == 7
    selected = validation["scheduled_baselines"]["selected"]
    assert selected["cagr"] == 0.17411179981597735
    assert selected["max_drawdown"] == -0.3877852536924954
    assert selected["utilization_fraction"] == 0.5509838998211091
    assert validation["passive_baselines"]["QQQ"]["cagr"] < selected["cagr"]
    assert validation["passive_baselines"]["QQQ"]["max_drawdown"] < selected[
        "max_drawdown"
    ]
    ...
    assert validation["multiplicity"]["current_lower_bound"] == 6315
...
def test_round63_trial_ledger_extension_is_append_only() -> None:
    result = audit_round63_extension(root=ROOT)
    assert result["passed"] is True
    assert result["current_lower_bound"] == 6315
