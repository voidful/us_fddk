...
from usfddk.global_trial_ledger_round55_extension import audit_round55_extension
...
def test_round55_protocol_is_frozen_and_self_bound() -> None:
    ...
    assert _load_protocol(ROOT)["status"] == "preregistered_oversold_reversal_diagnostic_only"
...
def test_round55_validation_is_negative_and_trade_blind() -> None:
    ...
    unsigned.pop("receipt_sha256")
    assert _canonical_sha256(unsigned) == claimed
    assert validation["status"] == "oversold_reversal_diagnostic_negative_survivorship_biased"
    assert validation["complete_event_counts"] == {"5": 234, "10": 233, "20": 233}
    assert validation["gate_summary"]["passed"] == 4
    ...
    assert validation["multiplicity"]["current_lower_bound"] == 6299
...
def test_round55_trial_ledger_extension_is_append_only() -> None:
    result = audit_round55_extension(root=ROOT)
    assert result["passed"] is True
    assert result["current_lower_bound"] == 6299
