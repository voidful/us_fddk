...
from scripts.probe_short_term_rf_treasury_bridge import _write_payload
...
def test_report_is_rendered_from_the_current_receipt() -> None:
    ...
    assert payload["status"] == "official_treasury_bridge_observed_formal_rf_still_blocked"
    assert payload["formal_rf_substitute"] is False
...
def test_probe_writer_keeps_artifact_site_data_and_report_in_lockstep(
    ...
    monkeypatch.setattr(
        "scripts.probe_short_term_rf_treasury_bridge.ARTIFACT", artifact
    )
    monkeypatch.setattr(
        "scripts.probe_short_term_rf_treasury_bridge.SITE_DATA", site_data
    )
    monkeypatch.setattr("scripts.probe_short_term_rf_treasury_bridge.REPORT", report)
