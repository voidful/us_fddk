...
EXTENSION_PATH = Path(
    "artifacts/short_term_global_trial_ledger_round58_extension.json"
)
...
PREDECESSOR_PATH = Path("artifacts/short_term_global_trial_ledger_round57_extension.json")
PREDECESSOR_SHA256 = (
    "528b3fb845c25c3aa1c5aebdb32a22b5bd2167471536aefeed06ac740cb49026"
)
PREDECESSOR_LOWER_BOUND = 6305
PREDECESSOR_CHAIN_HEAD = (
    "d96352cab0264815d6eff976d3d1644f307c4a5455dba360ff979d6264b0b778"
)
INCREMENT = 3
CURRENT_LOWER_BOUND = 6308
SEQUENCE = 18
FAMILY_ID = "round58_laggard_reversal_placebo_three_horizons"
EXPECTED_ENTRY_SHA256 = (
    "31708b8b049a970cebf89229b6efbcffb77cc64627d2ae36ad6712946f813e73"
)
EXPECTED_RECEIPT_SHA256 = (
    "892ed233e5ac6f921e5a5fff84fd2a77ca6fbe7798c5ee583ea2557fba80b82e"
)


class GlobalTrialLedgerRound58Error(ValueError):
...
def _fail(code: str, detail: str) -> None:
    raise GlobalTrialLedgerRound58Error(code, detail)
...
def _sha256_file(path: Path) -> str:
    try:
        ...
        _fail("trial_ledger_round58_source_missing", f"{path}: {type(exc).__name__}")
...
def _check_bindings(root: Path, bindings: object) -> int:
    if not isinstance(bindings, list) or len(bindings) != 4:
        _fail("trial_ledger_round58_binding_mismatch", "binding count drifted")
    labels: set[str] = set()
    for binding in bindings:
        if not isinstance(binding, dict) or set(binding) != {"label", "path", "sha256"}:
            _fail("trial_ledger_round58_binding_mismatch", "binding fields drifted")
        label = binding["label"]
        ...
        if (
            ...
            _fail("trial_ledger_round58_binding_mismatch", "binding value invalid")
        path = Path(path_value)
        if path.is_absolute() or ".." in path.parts:
            _fail("trial_ledger_round58_binding_mismatch", "binding path invalid")
        labels.add(label)
        if _sha256_file((root / path).resolve()) != claimed:
            _fail("trial_ledger_round58_binding_mismatch", f"source hash drifted: {path_value}")
    return len(bindings)


def audit_round58_extension(*, root: str | Path) -> dict[str, Any]:
    root_path = Path(root).resolve()
    if _sha256_file(root_path / PREDECESSOR_PATH) != PREDECESSOR_SHA256:
        _fail("trial_ledger_round58_predecessor_mismatch", "Round57 extension drifted")
    if _sha256_file(root_path / PROTOCOL_PATH) != EXPECTED_PROTOCOL_SHA256:
        _fail("trial_ledger_round58_protocol_mismatch", "protocol bytes drifted")
    try:
        extension = json.loads((root_path / EXTENSION_PATH).read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        _fail("trial_ledger_round58_schema_mismatch", type(exc).__name__)
    if not isinstance(extension, dict):
        _fail("trial_ledger_round58_schema_mismatch", "extension is not an object")
    entry = extension.get("entry")
    if not isinstance(entry, dict):
        _fail("trial_ledger_round58_schema_mismatch", "entry is missing")
    ...
    if claimed_entry != EXPECTED_ENTRY_SHA256 or _canonical_sha256(unsigned_entry) != claimed_entry:
        _fail("trial_ledger_round58_chain_mismatch", "entry hash drifted")
    if (
        ...
        _fail("trial_ledger_round58_arithmetic_mismatch", "entry arithmetic drifted")
    ...
    if (
        ...
        _fail("trial_ledger_round58_boundary_violation", "extension tip drifted")
    ...
    if claimed_receipt != EXPECTED_RECEIPT_SHA256 or _canonical_sha256(unsigned_extension) != claimed_receipt:
        _fail("trial_ledger_round58_chain_mismatch", "receipt hash drifted")
    return {
        "passed": True,
