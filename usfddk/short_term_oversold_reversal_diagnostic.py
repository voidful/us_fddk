...
SCHEMA_VERSION = "usfddk.short_term_oversold_reversal_diagnostic.v1"
PROTOCOL_PATH = Path("docs/SHORT_TERM_OVERSOLD_REVERSAL_DIAGNOSTIC_PROTOCOL.md")
PROTOCOL_RECEIPT_PATH = Path(
    "artifacts/short_term_oversold_reversal_diagnostic_protocol_receipt.json"
)
VALIDATION_PATH = Path(
    "artifacts/short_term_oversold_reversal_diagnostic_validation.json"
)
EXPECTED_PROTOCOL_SHA256 = (
    "39dfb053e8163263f9650d9c53945543bdc393b89492c6aaf01c551814f321e0"
)
EXPECTED_PROTOCOL_RECEIPT_SHA256 = (
    "bd4a03c9967f8742ceebd27574235459717e2aad657173d8eafb856d136cf78d"
)
...
GLOBAL_TRIAL_PRIOR_LOWER_BOUND = 6296
GLOBAL_TRIAL_INCREMENT = 3
GLOBAL_TRIAL_FAMILY_ID = "round55_oversold_reversal_three_horizons"


class OversoldReversalDiagnosticError(RuntimeError):
...
def _fail(code: str, detail: str) -> None:
    raise OversoldReversalDiagnosticError(code, detail)


def _load_protocol(root: Path) -> dict[str, Any]:
    if _sha256_file(root / PROTOCOL_PATH) != EXPECTED_PROTOCOL_SHA256:
        _fail("oversold_reversal_protocol_drift", "protocol bytes drifted")
    try:
        receipt = json.loads(
            (root / PROTOCOL_RECEIPT_PATH).read_text(encoding="utf-8")
        )
    except (OSError, json.JSONDecodeError) as exc:
        _fail("oversold_reversal_protocol_drift", type(exc).__name__)
    if not isinstance(receipt, dict):
        _fail("oversold_reversal_protocol_drift", "receipt is not an object")
    claimed = receipt.get("receipt_sha256")
    unsigned = dict(receipt)
    unsigned.pop("receipt_sha256", None)
    if claimed != EXPECTED_PROTOCOL_RECEIPT_SHA256 or _canonical_sha256(unsigned) != claimed:
        _fail("oversold_reversal_protocol_drift", "receipt hash drifted")
    expected = {
        "schema_version": 1,
        "research_round": 55,
        "status": "preregistered_oversold_reversal_diagnostic_only",
    ...
    if any(receipt.get(key) != value for key, value in expected.items()):
        _fail("oversold_reversal_protocol_drift", "fixed receipt field drifted")
    signal = receipt.get("signal")
    if signal != {
        "frequency": "weekly_completed_xnys",
        "dollar_volume_threshold_usd": DOLLAR_VOLUME_THRESHOLD,
        "lookback_low_sessions": LOW_LOOKBACK,
        "selection_count": SELECTION_COUNT,
        "short_return_sessions": SHORT_RETURN_SESSIONS,
        "short_return_threshold": SHORT_RETURN_THRESHOLD,
        "price_threshold_usd": PRICE_THRESHOLD,
    }:
        _fail("oversold_reversal_protocol_drift", "signal contract drifted")
    return receipt
...
def _event_rows(panel: Any, stocks: list[str], horizon: int) -> list[dict[str, float]]:
    ...
    short_return = close.pct_change(SHORT_RETURN_SESSIONS, fill_method=None)
    prior_low = close.rolling(LOW_LOOKBACK, min_periods=LOW_LOOKBACK).min().shift(1)
    ...
    for signal_date in weekly:
        ...
        signal = (
            base
            & (short_return.loc[signal_date] <= SHORT_RETURN_THRESHOLD)
            & (close.loc[signal_date] <= prior_low.loc[signal_date])
        )
        selected = sorted(
            signal.index[signal],
            key=lambda symbol: (float(short_return.loc[signal_date, symbol]), symbol),
        )[:SELECTION_COUNT]
...
def audit_oversold_reversal_diagnostic(
    *, repository_root: Path, snapshot_path: Path
    ...
    if snapshot.name != SNAPSHOT_FILENAME or not snapshot.is_file() or snapshot.is_symlink():
        _fail("oversold_reversal_snapshot_invalid", "snapshot path is not fixed")
    if _sha256_file(snapshot) != SNAPSHOT_ARCHIVE_SHA256:
        _fail("oversold_reversal_snapshot_invalid", "snapshot archive hash drifted")
    ...
    if (
        ...
        _fail("oversold_reversal_snapshot_invalid", "snapshot panel hash drifted")
    if _sha256_file(root / WATCHLIST_PATH) != WATCHLIST_SHA256:
        _fail("oversold_reversal_source_invalid", "watchlist hash drifted")
    ...
    if len(records) != WATCHLIST_COUNT or len(stocks) != WATCHLIST_COUNT:
        _fail("oversold_reversal_source_invalid", "watchlist coverage drifted")
    horizons: dict[str, Any] = {}
    complete_counts: dict[str, int] = {}
    for horizon in HORIZONS:
        rows = _event_rows(panel, stocks, horizon)
        if not rows:
            _fail("oversold_reversal_no_events", f"horizon {horizon} produced no events")
    ...
    result: dict[str, Any] = {
        ...
        "research_round": 55,
        "status": (
            "oversold_reversal_diagnostic_positive_survivorship_biased"
            if all(gates.values())
            else "oversold_reversal_diagnostic_negative_survivorship_biased"
        ),
        "source": {
            "protocol_sha256": EXPECTED_PROTOCOL_SHA256,
            "snapshot_filename": SNAPSHOT_FILENAME,
        ...
        "limitations": [
            ...
            "low_and_return_have_no_intraday_public_timestamp",
