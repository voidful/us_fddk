...
SCHEMA_VERSION = "usfddk.short_term_volume_breakout_top10_nonoverlap.v1"
PROTOCOL_PATH = Path("docs/SHORT_TERM_VOLUME_BREAKOUT_TOP10_NONOVERLAP_PROTOCOL.md")
PROTOCOL_RECEIPT_PATH = Path(
    "artifacts/short_term_volume_breakout_top10_nonoverlap_protocol_receipt.json"
)
VALIDATION_PATH = Path(
    "artifacts/short_term_volume_breakout_top10_nonoverlap_validation.json"
)
EXPECTED_PROTOCOL_SHA256 = (
    "2e0a52891d47471c0f9aa1ebf82a0e751fe46222d9704bd49d43fa143e9d1030"
)
EXPECTED_PROTOCOL_RECEIPT_SHA256 = (
    "819a848bd22dee9a0b44b27d478a5f97fbb705b5c64682e6bcd685da67c9555e"
)
ROUND_TRIP_COST_BPS = 20.0
STARTING_CAPITAL_USD = 1_000.0
HORIZON = 20
SELECTION_COUNT = 10
GLOBAL_TRIAL_PRIOR_LOWER_BOUND = 6313
GLOBAL_TRIAL_INCREMENT = 1
GLOBAL_TRIAL_FAMILY_ID = "round62_volume_breakout_top10_concentration"


class VolumeBreakoutTop10NonoverlapError(RuntimeError):
...
def _fail(code: str, detail: str) -> None:
    raise VolumeBreakoutTop10NonoverlapError(code, detail)


def _load_protocol(root: Path) -> dict[str, Any]:
    if _sha256_file(root / PROTOCOL_PATH) != EXPECTED_PROTOCOL_SHA256:
        _fail("volume_top10_protocol_drift", "protocol bytes drifted")
    try:
        receipt = json.loads((root / PROTOCOL_RECEIPT_PATH).read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        _fail("volume_top10_protocol_drift", type(exc).__name__)
    if not isinstance(receipt, dict):
        _fail("volume_top10_protocol_drift", "receipt is not an object")
    claimed = receipt.get("receipt_sha256")
    unsigned = dict(receipt)
    unsigned.pop("receipt_sha256", None)
    if claimed != EXPECTED_PROTOCOL_RECEIPT_SHA256 or _canonical_sha256(unsigned) != claimed:
        _fail("volume_top10_protocol_drift", "receipt hash drifted")
    expected = {
        "schema_version": 1,
        "research_round": 62,
        "status": "posthoc_volume_breakout_top10_concentration_diagnostic_only",
        "research_role": "concentration_stress_diagnostic",
    ...
    if any(receipt.get(key) != value for key, value in expected.items()):
        _fail("volume_top10_protocol_drift", "fixed receipt field drifted")
    if receipt.get("signal") != {
        ...
        _fail("volume_top10_protocol_drift", "signal contract drifted")
    if receipt.get("execution") != {
        ...
        _fail("volume_top10_protocol_drift", "execution contract drifted")
    if receipt.get("baselines") != {
        "event_schedule": ["eligible_pool", "complete_cohort", "SPY", "QQQ"],
        "passive": ["SPY", "QQQ"],
    }:
        _fail("volume_top10_protocol_drift", "baseline contract drifted")
    if receipt.get("gate_names") != [
        ...
        _fail("volume_top10_protocol_drift", "gate contract drifted")
    return receipt
...
def audit_volume_breakout_top10_nonoverlap(
    *, repository_root: Path, snapshot_path: Path
    ...
    if snapshot.name != SNAPSHOT_FILENAME or not snapshot.is_file() or snapshot.is_symlink():
        _fail("volume_top10_snapshot_invalid", "snapshot path is not fixed")
    if _sha256_file(snapshot) != SNAPSHOT_ARCHIVE_SHA256:
        _fail("volume_top10_snapshot_invalid", "snapshot archive hash drifted")
    ...
    if (
        ...
        _fail("volume_top10_snapshot_invalid", "snapshot panel hash drifted")
    if _sha256_file(root / WATCHLIST_PATH) != WATCHLIST_SHA256:
        _fail("volume_top10_source_invalid", "watchlist hash drifted")
    ...
    if len(records) != WATCHLIST_COUNT or len(stocks) != WATCHLIST_COUNT:
        _fail("volume_top10_source_invalid", "watchlist coverage drifted")
    index = pd.DatetimeIndex(panel.close.index).normalize()
    index = index[(index >= pd.Timestamp(START)) & (index <= pd.Timestamp(END))]
    candidates = _candidate_events(panel, stocks)
    accepted = _accepted_events(candidates)
    if not accepted:
        _fail("volume_top10_no_events", "no accepted events")
    scheduled = {
    ...
    result: dict[str, Any] = {
        ...
        "research_round": 62,
        "status": (
            "volume_breakout_top10_nonoverlap_concentration_positive_survivorship_biased"
            if all(gates.values())
            else "volume_breakout_top10_nonoverlap_concentration_negative_survivorship_biased"
        ),
        "research_role": "concentration_stress_diagnostic",
        "independent_first_seen_evidence": False,
        "strategy_rule_changed": True,
        ...
        "limitations": [
            ...
            "posthoc_top10_concentration_stress_is_not_independent_first_seen_evidence",
