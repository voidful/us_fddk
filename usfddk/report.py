...
def _equity_svg(results: list[BacktestResult], width: int = 980, height: int = 390) -> str:
    ...
    lines = [f'<svg viewBox="0 0 {width} {height}" role="img" aria-label="策略權益曲線">']
...
def _allocation_cards(results: list[BacktestResult]) -> str:
    ...
    for result in results:
        ...
        cards.append(
            ...
            '<p class="fine">這是最近月末收盤訊號的目標權重；模型成交時點為下一交易日開盤。</p></article>'
        )
    return "".join(cards)
...
def _candidate_evidence(validations: dict[str, Any]) -> str:
    ...
    if confirmed:
        ...
        explanation = (
            "原始報酬除以波動的數字為 1.07，但扣除 SHY 總報酬後只有 0.80。"
    ...
    <p class="fine">走勢外零利率／SHY 超額 Sharpe：{_num(walk.get("sharpe", float("nan")))}／{_num(walk.get("excess_sharpe_vs_shy", float("nan")))}。局部參數族超額報酬 PBO {_pct(pbo.get("pbo", float("nan")))}；越低越好。</p>
    """
...
def _growth_guard_evidence(validations: dict[str, Any]) -> str:
    ...
    <div class="grid4 evidence-grid">
      ...
      <div class="kpi"><span>最大回撤</span><strong>{_pct(strategy.get("max_drawdown", float("nan")))}</strong><small>SPY {_pct(spy.get("max_drawdown", float("nan")))}</small></div>
      <div class="kpi"><span>5 年滾動勝率</span><strong>{_pct(rolling.get("cagr_win_fraction", float("nan")))}</strong><small>相對 SPY</small></div>
      <div class="kpi"><span>超額報酬 NW t</span><strong>{_num(nw.get("t_stat", float("nan")))}</strong><small>需達 1.96；DSR {_pct(dsr.get("probability", float("nan")))}</small></div>
    </div>
    <p class="fine">QQQ 買進持有 CAGR {_pct(qqq.get("cagr", float("nan")))}、MDD {_pct(qqq.get("max_drawdown", float("nan")))}。候選是以降低回撤換取部分 QQQ 報酬，不宣稱每個期間都領先。</p>
    """
...
def _volatility_guard_evidence(validations: dict[str, Any]) -> str:
    ...
    <div class="grid4 evidence-grid">
      ...
      <div class="kpi"><span>最大回撤</span><strong>{_pct(strategy.get("max_drawdown", float("nan")))}</strong><small>v1 {_pct(incumbent.get("max_drawdown", float("nan")))}</small></div>
...
def _v3_evidence(validations: dict[str, Any]) -> str:
    ...
    v3 在成長 regime 持有 100% QQQ；只有 12 個月趨勢連續兩個月為負後，才以 18%／21 日波動政策降風險。代理期的有效滾動勝率失敗，因此只建立隔離 Paper。</div>
    <div class="grid4 evidence-grid">
      ...
      <div class="kpi"><span>v3 最大回撤</span><strong>{_pct(strategy.get("max_drawdown", float("nan")))}</strong><small>QQQ {_pct(qqq.get("max_drawdown", float("nan")))}</small></div>
      <div class="kpi"><span>96/4 曝險基準</span><strong>{_pct(matched.get("cagr", float("nan")))}</strong><small>MDD {_pct(matched.get("max_drawdown", float("nan")))}</small></div>
    ...
    sections = (
        (
            ...
            item.get("historical_gates", {}),
            {
                ...
                "drawdown_improvement_at_least_15pp_vs_qqq": "最大回撤至少改善 15pp",
                "both_ten_year_halves_beat_qqq": "前後十年都高於 QQQ",
                "still_beats_qqq_at_25bps": "25 bps 仍高於 QQQ",
                "positive_average_daily_active_return_vs_qqq": "平均每日主動報酬為正",
                "confirmation_neighborhood_all_beats_qqq": "1／2／3 月確認全期皆高於 QQQ",
                "confirmation_neighborhood_all_improves_drawdown_10pp": "1／2／3 月確認回撤皆改善 10pp",
        ...
        (
            ...
            item.get("exposure_control_gates", {}),
            {
                ...
                "drawdown_improvement_at_least_10pp_vs_matched_96_4": "最大回撤至少改善 10pp",
                ...
                "positive_average_daily_active_return_vs_matched_96_4": "平均每日主動報酬為正",
            },
        ),
        (
            "1986–2006 代理期",
            item.get("proxy_validation", {}).get("gates", {}),
            {
                ...
                "drawdown_improvement_at_least_10pp": "最大回撤至少改善 10pp",
                "rolling_five_year_win_rate_at_least_60pct": "5 年有效勝率至少 60%",
                "still_beats_ndx_at_50bps": "50 bps 仍高於 NDX",
                "positive_average_daily_active_return": "平均每日主動報酬為正",
            },
        ),
    )
    rows: list[str] = []
...
def _volatility_gate_rows(validations: dict[str, Any]) -> str:
    labels = {
        ...
        "drawdown_improvement_at_least_15pp": "最大回撤改善至少 15 個百分點",
        ...
        "improves_incumbent_cagr_and_drawdown": "同時改善 v1 的 CAGR 與回撤",
...
def _exposure_control_gate_rows(validations: dict[str, Any]) -> str:
    labels = {
        ...
        "drawdown_improvement_at_least_10pp_vs_passive_90_10": ("最大回撤至少改善 10 個百分點"),
...
def _growth_gate_rows(validations: dict[str, Any]) -> str:
    labels = {
        "full_cagr_at_least_spy_plus_3pp": "全期 CAGR 至少領先 SPY 3 個百分點",
        "sharpe_above_spy": "Sharpe 高於 SPY",
        "drawdown_improvement_at_least_10pp": "最大回撤改善至少 10 個百分點",
        "both_ten_year_halves_beat_spy": "前後兩個十年都勝過 SPY",
...
def build_cross_market_report(destination: str | Path, audit: dict[str, Any]) -> Path:
    ...
    gate_labels = {
        ...
        "at_least_four_drawdown_improvements_of_10pp": "至少 4/5 市場回撤改善 10 個百分點",
        "at_least_four_cost_50bps_wins": "至少 4/5 市場在 50 bps 仍勝出",
        "rolling_median_and_three_markets_at_least_60pct": "滾動勝率中位數與至少 3 市場達 60%",
        "pooled_active_newey_west_t_at_least_1_96": "等權主動報酬 NW t 至少 1.96",
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font:15px/1.55 ui-sans-serif,system-ui,-apple-system,"PingFang TC","Noto Sans TC",sans-serif}main{max-width:1160px;margin:auto;padding:42px 22px 80px}header{border-bottom:3px solid var(--ink);padding-bottom:24px}.eyebrow{font-weight:800;letter-spacing:.13em;color:var(--red)}h1{font-size:clamp(36px,7vw,72px);line-height:1;margin:8px 0 16px;letter-spacing:-.05em}h2{margin:0 0 14px;font-size:24px}.lead{max-width:800px;font-size:18px}.verdict{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin:24px 0}.card,.panel{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:20px}.card strong{display:block;font-size:34px}.card span,.note,small{display:block;color:var(--muted)}.fail{color:var(--red);font-weight:750}.pass{color:var(--green);font-weight:750}.panel{margin-top:16px}.warning{border-left:5px solid var(--red);background:#f8e7e2;padding:16px 18px;border-radius:8px;margin-top:16px}.table-wrap{overflow:auto}table{width:100%;border-collapse:collapse;min-width:860px}th,td{text-align:right;border-bottom:1px solid var(--line);padding:11px 9px;white-space:nowrap}th:first-child{text-align:left}thead th{color:var(--muted);font-size:12px}code{font-size:11px;word-break:break-all;white-space:normal}.footer{color:var(--muted);font-size:12px;margin-top:24px}@media(max-width:720px){.verdict{grid-template-columns:1fr}main{padding:24px 12px 56px}.panel{padding:14px}}
    """
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v3 五市場機制驗證</title><style>{css}</style></head><body><main>
    ...
    <section class="verdict"><article class="card"><span>完整期 CAGR 勝出</span><strong class="fail">{counts["full_cagr"]} / 5</strong><small>硬門檻至少 4 / 5</small></article><article class="card"><span>跨市場等權主動報酬 NW t</span><strong class="fail">{_num(pooled["newey_west"]["t_stat"])}</strong><small>硬門檻至少 1.96</small></article><article class="card"><span>五年滾動勝率中位數</span><strong class="fail">{_pct(audit["rolling_five_year_win_rate_median"], 1)}</strong><small>硬門檻至少 60%</small></article><article class="card"><span>6,100 次搜尋後 DSR 機率</span><strong class="fail">{_pct(pooled["deflated_sharpe"]["probability"], 4)}</strong><small>選擇偏誤懲罰</small></article></section>
    <div class="warning"><b>研究決定：保留負結果，不調參救援。</b><br>v3 仍可在隔離 Paper 帳戶累積前瞻紀錄，但本結果降低而不是提高其可信度；不替換 v2，也不成為參考交易候選。</div>
    <section class="panel"><h2>五市場逐一收據</h2><p class="note">CAGR 差、回撤改善與成本差均為策略減 buy-and-hold；前／後半期顯示兩段固定期間的 CAGR 差。</p><div class="table-wrap"><table><thead><tr><th>市場</th><th>v3 CAGR</th><th>買進持有</th><th>CAGR 差</th><th>Sharpe v3 / 基準</th><th>回撤改善</th><th>50 bps CAGR 差</th><th>5 年滾動勝率</th><th>前／後半期</th><th>NW t</th></tr></thead><tbody>{"".join(market_rows)}</tbody></table></div></section>
    ...
    <section class="panel"><h2>資料與協議收據</h2><p>協議 SHA-256：<code>{html.escape(protocol["sha256"])}</code><br>固定驗證期：{html.escape(audit["period"]["start"])}–{html.escape(audit["period"]["end"])}<br>成本：10 bps；壓力成本：50 bps；防守資產：零報酬 CASH。</p><div class="table-wrap"><table><thead><tr><th>代號</th><th>暖機 sessions</th><th>面板 SHA-256</th><th>封存檔 SHA-256</th></tr></thead><tbody>{"".join(receipt_rows)}</tbody></table></div></section>
    <p class="footer">價格指數不是總報酬指數；策略與基準採相同價格口徑。研究與教育用途，不構成投資建議。</p>
    </main></body></html>"""
...
def build_style_rotation_report(destination: str | Path, audit: dict[str, Any]) -> Path:
    ...
    gate_labels = {
        ...
        "03_trade_drawdown_improves_spy_by_10pp": "最大回撤相對 SPY 改善至少 10 個百分點",
        ...
        "08_trade_qqq_opportunity_cost_and_drawdown": "QQQ 報酬機會成本不超過 1pp 且回撤改善 10pp",
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font:15px/1.55 ui-sans-serif,system-ui,-apple-system,"PingFang TC","Noto Sans TC",sans-serif}main{max-width:1180px;margin:auto;padding:42px 22px 80px}header{border-bottom:3px solid var(--ink);padding-bottom:24px}.eyebrow{font-weight:800;letter-spacing:.13em;color:var(--red)}h1{font-size:clamp(34px,7vw,68px);line-height:1.03;margin:8px 0 16px;letter-spacing:-.05em}h2{margin:0 0 14px;font-size:24px}.lead{max-width:850px;font-size:18px}.verdict{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin:24px 0}.card,.panel{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:20px}.card strong{display:block;font-size:32px}.card span,.note,small{display:block;color:var(--muted)}.fail{color:var(--red);font-weight:750}.pass{color:var(--green);font-weight:750}.panel{margin-top:16px}.warning{border-left:5px solid var(--red);background:#f8e7e2;padding:16px 18px;border-radius:8px;margin-top:16px}.table-wrap{overflow:auto}table{width:100%;border-collapse:collapse;min-width:800px}th,td{text-align:right;border-bottom:1px solid var(--line);padding:11px 9px;white-space:nowrap}th:first-child{text-align:left}thead th{color:var(--muted);font-size:12px}code{font-size:11px;word-break:break-all;white-space:normal}.footer{color:var(--muted);font-size:12px;margin-top:24px}@media(max-width:900px){.verdict{grid-template-columns:1fr 1fr}}@media(max-width:600px){.verdict{grid-template-columns:1fr}main{padding:24px 12px 56px}.panel{padding:14px}}
    ...
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v4 股權風格輪動研究收據</title><style>{css}</style></head><body><main>
    <header><div class="eyebrow">PREDECLARED STYLE-ROTATION AUDIT</div><h1>回撤較淺，<br>但報酬門檻失敗。</h1><p class="lead">v4 在可交易 20 年中把 SPY 最大回撤改善 {_pct(trade["comparisons"]["market"]["drawdown_improvement"], 1)}，Sharpe 也較高；但 CAGR 落後 SPY {_pct(abs(trade["comparisons"]["market"]["cagr_difference"]), 2)}，後十年與五年滾動結果明顯失敗。舊代理又沒有足夠資料，因此不能建立 Paper 帳戶。</p></header>
    <section class="verdict"><article class="card"><span>14 道硬門檻</span><strong class="fail">{audit["passed_gate_count"]} / {audit["required_gate_count"]}</strong><small>必須全部通過</small></article><article class="card"><span>策略 / SPY CAGR</span><strong>{_pct(strategy["cagr"], 2)} / {_pct(trade["benchmark_metrics"]["market"]["cagr"], 2)}</strong><small>主成本 10 bps</small></article><article class="card"><span>策略最大回撤</span><strong class="pass">{_pct(strategy["max_drawdown"], 1)}</strong><small>SPY {_pct(trade["benchmark_metrics"]["market"]["max_drawdown"], 1)}</small></article><article class="card"><span>目前研究訊號</span><strong>{html.escape(current or "全數防守")}</strong><small>不是交易建議</small></article></section>
    <div class="warning"><b>研究決定：不啟動 v4 Paper，也不調參救援。</b><br>50 bps 成本、後十年、五年滾動、Newey–West 與 QQQ 機會成本均未過關；`^RLG`／`^RLV` 只有 2002 年後資料，舊代理六道門檻依協議全部失敗。</div>
    <section class="panel"><h2>20 年主樣本：同口徑比較</h2><p class="note">所有數字都用 2006-07-31–2026-07-31、月末訊號、下一交易日開盤與相同成本。</p><div class="table-wrap"><table><thead><tr><th>基準</th><th>策略 / 基準 CAGR</th><th>CAGR 差</th><th>策略 / 基準 Sharpe</th><th>策略 / 基準 MDD</th><th>回撤改善</th><th>50 bps CAGR 差</th><th>5 年勝率</th><th>5 年差中位數</th><th>NW t</th></tr></thead><tbody>{"".join(comparison_rows)}</tbody></table></div></section>
    ...
    <section class="panel"><h2>舊代理資料門檻</h2><p class="note">協議要求 1996-07-31 前至少 273 個有效 session；不允許事後換代號。</p><div class="table-wrap"><table><thead><tr><th>代號</th><th>有效 sessions</th><th>首筆</th><th>末筆</th><th>起算前暖機</th></tr></thead><tbody>{proxy_rows}</tbody></table></div></section>
    ...
    <section class="panel"><h2>快照與協議收據</h2><p>協議 SHA-256：<code>{html.escape(audit["protocol"]["sha256"])}</code></p><div class="table-wrap"><table><thead><tr><th>資料</th><th>期間</th><th>列數</th><th>面板 SHA-256</th><th>封存檔 SHA-256</th></tr></thead><tbody>{receipt_rows}</tbody></table></div></section>
    <p class="footer">調整後 ETF 價格與價格指數代理不可跨段串接。歷史回測、研究與教育用途，不構成投資建議。</p>
    </main></body></html>"""
    destination.write_text(content, encoding="utf-8")
    return destination
...
def build_three_clock_report(destination: str | Path, audit: dict[str, Any]) -> Path:
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font:15px/1.55 ui-sans-serif,system-ui,-apple-system,"PingFang TC","Noto Sans TC",sans-serif}main{max-width:1180px;margin:auto;padding:42px 22px 80px}header{border-bottom:3px solid var(--ink);padding-bottom:24px}.eyebrow{font-weight:800;letter-spacing:.13em;color:var(--red)}h1{font-size:clamp(34px,7vw,68px);line-height:1.03;margin:8px 0 16px;letter-spacing:-.05em}h2{margin:0 0 14px;font-size:24px}.lead{max-width:870px;font-size:18px}.verdict{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin:24px 0}.card,.panel{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:20px}.card strong{display:block;font-size:30px}.card span,.note,small{display:block;color:var(--muted)}.fail{color:var(--red);font-weight:750}.pass{color:var(--green);font-weight:750}.panel{margin-top:16px}.warning{border-left:5px solid var(--red);background:#f8e7e2;padding:16px 18px;border-radius:8px;margin-top:16px}.table-wrap{overflow:auto}table{width:100%;border-collapse:collapse;min-width:900px}th,td{text-align:right;border-bottom:1px solid var(--line);padding:11px 9px;white-space:nowrap}th:first-child{text-align:left}thead th{color:var(--muted);font-size:12px}code{font-size:11px;word-break:break-all;white-space:normal}.footer{color:var(--muted);font-size:12px;margin-top:24px}@media(max-width:900px){.verdict{grid-template-columns:1fr 1fr}}@media(max-width:600px){.verdict{grid-template-columns:1fr}main{padding:24px 12px 56px}.panel{padding:14px}}
    """
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v5 三時鐘集成研究收據</title><style>{css}</style></head><body><main>
    <header><div class="eyebrow">FROZEN THREE-CLOCK ENSEMBLE AUDIT</div><h1>近期幾乎追平 QQQ，<br>外部驗證仍失敗。</h1><p class="lead">三個固定三分之一袖套在 2006–2026 得到 {_pct(strategy["cagr"], 2)} CAGR，QQQ 為 {_pct(main["benchmark_metrics"]["opportunity"]["cagr"], 2)}，回撤改善 {_pct(opportunity["drawdown_improvement"], 1)}。但這不是結論：舊年代滾動一致性與五市場泛化仍失敗，所以不能建立 Paper。</p></header>
    ...
    <div class="warning"><b>研究決定：不建立 v5 Paper，不改袖套比例救援。</b><br>近期主樣本通過 7/9 道，但 1986–2006 滾動勝率只有約 37%，五市場完整期同時勝兩基準只有 {counts["full_cagr_beats_both"]}/5；五市場等權主動報酬相對 buy-and-hold 為 {_pct(pooled["market"]["newey_west"]["annualized"], 2)}，NW t = {_num(pooled["market"]["newey_west"]["t_stat"])}。</div>
    <section class="panel"><h2>近期 20 年：看似接近，但 matched alpha 很弱</h2><div class="table-wrap"><table><thead><tr><th>基準</th><th>策略 / 基準 CAGR</th><th>CAGR 差</th><th>策略 / 基準 Sharpe</th><th>策略 / 基準 MDD</th><th>回撤改善</th><th>50 bps CAGR 差</th><th>5 年勝率</th><th>NW t</th><th>DSR</th></tr></thead><tbody>{"".join(main_rows)}</tbody></table></div></section>
    ...
    <section class="panel"><h2>五市場：只有單一市場完整期同勝兩基準</h2><div class="table-wrap"><table><thead><tr><th>市場</th><th>策略 CAGR</th><th>買進持有</th><th>固定 95/5</th><th>相對兩基準</th><th>回撤改善</th><th>5 年勝率（買進／95/5）</th></tr></thead><tbody>{"".join(market_rows)}</tbody></table></div></section>
...
def build_industry_tilt_report(destination: str | Path, audit: dict[str, Any]) -> Path:
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font:15px/1.55 ui-sans-serif,system-ui,-apple-system,"PingFang TC","Noto Sans TC",sans-serif}main{max-width:1180px;margin:auto;padding:42px 22px 80px}header{border-bottom:3px solid var(--ink);padding-bottom:24px}.eyebrow{font-weight:800;letter-spacing:.13em;color:var(--red)}h1{font-size:clamp(34px,7vw,68px);line-height:1.03;margin:8px 0 16px;letter-spacing:-.05em}h2{margin:0 0 14px;font-size:24px}.lead{max-width:890px;font-size:18px}.verdict{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin:24px 0}.card,.panel{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:20px}.card strong{display:block;font-size:30px}.card span,.note,small{display:block;color:var(--muted)}.fail{color:var(--red);font-weight:750}.pass{color:var(--green);font-weight:750}.panel{margin-top:16px}.warning{border-left:5px solid var(--red);background:#f8e7e2;padding:16px 18px;border-radius:8px;margin-top:16px}.table-wrap{overflow:auto}table{width:100%;border-collapse:collapse;min-width:900px}th,td{text-align:right;border-bottom:1px solid var(--line);padding:11px 9px;white-space:nowrap}th:first-child{text-align:left}thead th{color:var(--muted);font-size:12px}code{font-size:11px;word-break:break-all;white-space:normal}.footer{color:var(--muted);font-size:12px;margin-top:24px}@media(max-width:900px){.verdict{grid-template-columns:1fr 1fr}}@media(max-width:600px){.verdict{grid-template-columns:1fr}main{padding:24px 12px 56px}.panel{padding:14px}}
    """
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v6 產業動能研究收據</title><style>{css}</style></head><body><main>
    <header><div class="eyebrow">FROZEN INDUSTRY-MOMENTUM AUDIT</div><h1>長期代理有效，<br>可交易主期仍落後。</h1><p class="lead">v6 在新資料下載前已把規則與 22 道門檻鎖定。1927–2005 代理期支持產業動能機制，但真正可交易的 2006–2026 ETF 策略 CAGR 只有 {_pct(strategy["cagr"], 2)}，低於 SPY 的 {_pct(spy["cagr"], 2)}，也低於相同每月權益曝險對照的 {_pct(matched["cagr"], 2)}。實務主測試優先，因此本候選淘汰。</p></header>
    <section class="verdict"><article class="card"><span>22 道事前硬門檻</span><strong class="fail">{audit["passed_gate_count"]} / {audit["required_gate_count"]}</strong><small>必須全部通過</small></article><article class="card"><span>ETF 主期策略 / SPY</span><strong>{_pct(strategy["cagr"], 2)} / {_pct(spy["cagr"], 2)}</strong><small>相差 {_pct(strategy["cagr"] - spy["cagr"], 2)}</small></article><article class="card"><span>策略 / matched</span><strong>{_pct(strategy["cagr"], 2)} / {_pct(matched["cagr"], 2)}</strong><small>選產業沒有增加淨報酬</small></article><article class="card"><span>研究狀態</span><strong class="fail">不開 Paper</strong><small>不調參救援</small></article></section>
    <div class="warning"><b>研究決定：封存負結果，網站主訊號與 Paper 都不改。</b><br>目前規則算出的最後歷史權重是 {html.escape(current)}，但策略已被 11 道失敗門檻淘汰，這組權重不可照單。主要失敗包括 CAGR、同曝險 Sharpe、50 bps 成本、前後半期、滾動一致性與統計門檻。</div>
    <section class="panel"><h2>三段資料，同一個研究問題</h2><p class="note">表內依序為策略／廣泛市場／selection-matched control。代理期只能驗證機制，不能取代 ETF 可交易結果。</p><div class="table-wrap"><table><thead><tr><th>資料段</th><th>策略 CAGR</th><th>市場 CAGR</th><th>matched CAGR</th><th>Sharpe：策略 / 市場 / matched</th><th>最大回撤：策略 / 市場 / matched</th></tr></thead><tbody>{period_rows}</tbody></table></div></section>
    ...
    <section class="panel"><h2>資料與不可竄改收據</h2><p>協議 SHA-256：<code>{html.escape(protocol["sha256"])}</code><br>ETF panel SHA-256：<code>{html.escape(etf["panel_sha256"])}</code><br>ETF archive SHA-256：<code>{html.escape(etf["archive_sha256"])}</code><br>French 10 Industry ZIP：<code>{html.escape(french["industry"]["sha256"])}</code><br>French Factors ZIP：<code>{html.escape(french["factors"]["sha256"])}</code></p></section>
    <p class="footer">歷史回測、研究與教育用途，不構成投資建議。過去績效不保證未來結果。</p>
    </main></body></html>"""
    destination.write_text(content, encoding="utf-8")
    return destination
...
def build_relative_growth_report(destination: str | Path, audit: dict[str, Any]) -> Path:
    ...
    period_rows = "".join(
        [
            period_row("可交易 ETF 主期", main, "2006-07-31–2026-07-31；總報酬 ETF"),
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font:15px/1.55 ui-sans-serif,system-ui,-apple-system,"PingFang TC","Noto Sans TC",sans-serif}main{max-width:1180px;margin:auto;padding:42px 22px 80px}header{border-bottom:3px solid var(--ink);padding-bottom:24px}.eyebrow{font-weight:800;letter-spacing:.13em;color:var(--gold)}h1{font-size:clamp(34px,7vw,68px);line-height:1.03;margin:8px 0 16px;letter-spacing:-.05em}h2{margin:0 0 14px;font-size:24px}.lead{max-width:900px;font-size:18px}.verdict{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin:24px 0}.card,.panel{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:20px}.card strong{display:block;font-size:30px}.card span,.note,small{display:block;color:var(--muted)}.fail{color:var(--red);font-weight:750}.pass{color:var(--green);font-weight:750}.panel{margin-top:16px}.warning{border-left:5px solid var(--red);background:#f8e7e2;padding:16px 18px;border-radius:8px;margin-top:16px}.success{border-left-color:var(--green);background:#e5f2e9}.table-wrap{overflow:auto}table{width:100%;border-collapse:collapse;min-width:920px}th,td{text-align:right;border-bottom:1px solid var(--line);padding:11px 9px;white-space:nowrap}th:first-child{text-align:left}thead th{color:var(--muted);font-size:12px}code{font-size:11px;word-break:break-all;white-space:normal}.footer{color:var(--muted);font-size:12px;margin-top:24px}@media(max-width:900px){.verdict{grid-template-columns:1fr 1fr}}@media(max-width:600px){.verdict{grid-template-columns:1fr}main{padding:24px 12px 56px}.panel{padding:14px}}
    ...
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v7 相對成長衛星研究收據</title><style>{css}</style></head><body><main>
    <header><div class="eyebrow">FROZEN RELATIVE-GROWTH AUDIT</div><h1>{headline}</h1><p class="lead">v7 永久保留 50% 廣泛市場核心；只有 QQQ 的 12–1 動能領先 SPY 且仍站在 200 日均線上時，另一半才持有 QQQ，否則轉到 SHY。matched control 每月持有相同股票曝險，用來分辨「降低風險的配置政策」和「選 QQQ 真正帶來的超額報酬」。</p></header>
    ...
    <section class="panel"><h2>兩段不重疊資料</h2><div class="table-wrap"><table><thead><tr><th>資料段</th><th>策略 CAGR</th><th>市場 CAGR</th><th>matched CAGR</th><th>最大回撤：策略 / 市場 / matched</th><th>對市場 NW t</th><th>對 matched NW t</th></tr></thead><tbody>{period_rows}</tbody></table></div></section>
    ...
    <section class="panel"><h2>不可竄改資料收據</h2><p>協議 SHA-256：<code>{html.escape(audit["protocol"]["sha256"])}</code><br>主 ETF panel：<code>{html.escape(audit["data_receipts"]["main"]["panel_sha256"])}</code><br>Nasdaq-100 archive：<code>{html.escape(audit["data_receipts"]["ndx"]["archive_sha256"])}</code><br>S&amp;P 500 archive：<code>{html.escape(audit["data_receipts"]["gspc"]["archive_sha256"])}</code></p></section>
    <p class="footer">歷史回測、研究與教育用途，不構成投資建議。價格指數代理不含股息，不能冒充 ETF 可交易績效；過去績效不保證未來結果。</p>
    </main></body></html>"""
    destination.write_text(content, encoding="utf-8")
    return destination
...
def build_always_invested_report(destination: str | Path, audit: dict[str, Any]) -> Path:
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font:15px/1.55 ui-sans-serif,system-ui,-apple-system,"PingFang TC","Noto Sans TC",sans-serif}main{max-width:1180px;margin:auto;padding:42px 22px 80px}header{border-bottom:3px solid var(--ink);padding-bottom:24px}.eyebrow{font-weight:800;letter-spacing:.13em;color:var(--gold)}h1{font-size:clamp(34px,7vw,68px);line-height:1.03;margin:8px 0 16px;letter-spacing:-.05em}h2{margin:0 0 14px;font-size:24px}.lead{max-width:900px;font-size:18px}.verdict{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin:24px 0}.card,.panel{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:20px}.card strong{display:block;font-size:30px}.card span,.note,small{display:block;color:var(--muted)}.fail{color:var(--red);font-weight:750}.pass{color:var(--green);font-weight:750}.panel{margin-top:16px}.warning{border-left:5px solid var(--red);background:#f8e7e2;padding:16px 18px;border-radius:8px;margin-top:16px}.success{border-left-color:var(--green);background:#e5f2e9}.table-wrap{overflow:auto}table{width:100%;border-collapse:collapse;min-width:980px}th,td{text-align:right;border-bottom:1px solid var(--line);padding:11px 9px;white-space:nowrap}th:first-child{text-align:left}thead th{color:var(--muted);font-size:12px}code{font-size:11px;word-break:break-all;white-space:normal}.footer{color:var(--muted);font-size:12px;margin-top:24px}@media(max-width:900px){.verdict{grid-template-columns:1fr 1fr}}@media(max-width:600px){.verdict{grid-template-columns:1fr}main{padding:24px 12px 56px}.panel{padding:14px}}
    ...
    if paper_ok:
        ...
        headline = "仍未跨過 Paper，<br>不把成長曝險當 alpha。"
        decision = f"16 道 Paper 入口有 {len(paper_failed)} 道失敗；封存結果、不建立帳戶。"
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v8 永遠持股相對成長研究收據</title><style>{css}</style></head><body><main>
    <header><div class="eyebrow">FROZEN ALWAYS-INVESTED AUDIT</div><h1>{headline}</h1><p class="lead">v8 永遠持有 100% 股票：50% 固定 SPY，另一半只有在 QQQ 相對強勢且仍在 200 日均線上時才換成 QQQ，否則也持有 SPY。因此和 SPY 基準的差異只來自成長傾斜，不再混入轉債券所降低的市場曝險。</p></header>
    <section class="verdict"><article class="card"><span>Paper 經濟入口</span><strong class={"pass" if paper_ok else "fail"}>{audit["paper_entry_passed_gate_count"]} / {audit["paper_entry_required_gate_count"]}</strong><small>全過才准開新帳戶</small></article><article class="card"><span>全部歷史門檻</span><strong class={"pass" if history_ok else "fail"}>{audit["passed_gate_count"]} / {audit["required_gate_count"]}</strong><small>含 NW 與 PSR</small></article><article class="card"><span>主期策略 / SPY CAGR</span><strong>{_pct(strategy["cagr"], 2)} / {_pct(spy["cagr"], 2)}</strong><small>差 {_pct(strategy["cagr"] - spy["cagr"], 2)}</small></article><article class="card"><span>最後歷史權重</span><strong>{html.escape(current)}</strong><small>{"只供 Paper 起點" if paper_ok else "淘汰研究，不可照單"}</small></article></section>
    ...
    <section class="panel"><h2>同樣 100% 股票，才是公平問題</h2><p>SPY 與 v8 每一天都維持完整股票曝險。v8 若勝出，才可能歸因於 QQQ 相對強勢傾斜；它不再能靠持有 SHY、少承擔市場風險來改善回撤或 Sharpe。固定 50/50 與 QQQ 仍列作機會成本背景，但 Paper 入口的 hard benchmark 是 SPY。</p></section>
    <section class="panel"><h2>近期與舊期分開驗證</h2><div class="table-wrap"><table><thead><tr><th>資料段</th><th>策略 CAGR</th><th>市場 CAGR</th><th>年化差</th><th>最大回撤：策略 / 市場</th><th>五年勝率</th><th>NW t</th><th>PSR</th></tr></thead><tbody>{period_rows}</tbody></table></div></section>
    <section class="panel"><h2>二十道凍結歷史門檻</h2><p class="note">經濟／跨期入口通過 {audit["paper_entry_passed_gate_count"]} / {audit["paper_entry_required_gate_count"]}；全部歷史通過 {audit["passed_gate_count"]} / {audit["required_gate_count"]}。Paper 是取得新資料，不是實金授權。</p><div class="table-wrap"><table><thead><tr><th>固定檢查鍵</th><th>結果</th></tr></thead><tbody>{gate_rows}</tbody></table></div></section>
...
def build_low_turnover_report(destination: str | Path, audit: dict[str, Any]) -> Path:
    ...
    period_rows = "".join(
        [
            period_row(
                "可交易 ETF 主期",
                audit["main"],
                "2006-07-31–2026-07-31；SPY／QQQ 調整後總報酬",
            ),
            period_row(
                "已看過的舊代理",
                audit["old_proxy"],
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font:15px/1.55 ui-sans-serif,system-ui,-apple-system,"PingFang TC","Noto Sans TC",sans-serif}main{max-width:1220px;margin:auto;padding:42px 22px 80px}header{border-bottom:3px solid var(--ink);padding-bottom:24px}.eyebrow{font-weight:800;letter-spacing:.13em;color:var(--gold)}h1{font-size:clamp(34px,7vw,68px);line-height:1.03;margin:8px 0 16px;letter-spacing:-.05em}h2{margin:0 0 14px;font-size:24px}.lead{max-width:940px;font-size:18px}.verdict{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin:24px 0}.card,.panel{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:20px}.card strong{display:block;font-size:30px}.card span,.note,small{display:block;color:var(--muted)}.fail{color:var(--red);font-weight:750}.pass{color:var(--green);font-weight:750}.panel{margin-top:16px}.warning{border-left:5px solid var(--red);background:#f8e7e2;padding:16px 18px;border-radius:8px;margin-top:16px}.success{border-left-color:var(--green);background:#e5f2e9}.table-wrap{overflow:auto}table{width:100%;border-collapse:collapse;min-width:1120px}th,td{text-align:right;border-bottom:1px solid var(--line);padding:11px 9px;white-space:nowrap}th:first-child{text-align:left}thead th{color:var(--muted);font-size:12px}code{font-size:11px;word-break:break-all;white-space:normal}.footer{color:var(--muted);font-size:12px;margin-top:24px}@media(max-width:900px){.verdict{grid-template-columns:1fr 1fr}}@media(max-width:600px){.verdict{grid-template-columns:1fr}main{padding:24px 12px 56px}.panel{padding:14px}}
    ...
    if paper_ok:
        ...
        decision = "23 道經濟、跨期、曝險與資料門檻全過；可以建立全新隔離 Paper。" + (
            "六道歷史統計也全過，但仍不能跳過前瞻期。"
            if history_ok
        ...
        decision = (
            ...
            "不建立帳戶，也不回頭調整 60/40 或門檻。"
        )
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v9 低換手外部驗證研究收據</title><style>{css}</style></head><body><main>
    <header><div class="eyebrow">FROZEN LOW-TURNOVER EXTERNAL AUDIT</div><h1>{headline}</h1><p class="lead">v9 永遠持有 100% 股票：平時 100% 廣泛市場；只有成長市場的 12–1 相對強度領先、且仍在 200 日均線上時，才改成 60% 核心／40% 成長。每月檢查，但只在狀態改變時成交，兩次切換間讓持倉自然漂移。</p></header>
    <section class="verdict"><article class="card"><span>Paper 入口</span><strong class={"pass" if paper_ok else "fail"}>{audit["paper_entry_passed_gate_count"]} / {audit["paper_entry_required_gate_count"]}</strong><small>三段經濟門檻全過才算</small></article><article class="card"><span>全部歷史門檻</span><strong class={"pass" if history_ok else "fail"}>{audit["passed_gate_count"]} / {audit["required_gate_count"]}</strong><small>另含 NW 與 PSR</small></article><article class="card"><span>主期策略 / SPY CAGR</span><strong>{_pct(strategy["cagr"], 2)} / {_pct(spy["cagr"], 2)}</strong><small>50 bps 差 {_pct(main["cost_50bps"]["cagr_difference"], 2)}</small></article><article class="card"><span>最新研究狀態</span><strong>{html.escape(policy)}</strong><small>{"只供 Paper 新帳戶" if paper_ok else "淘汰研究，不可照單"}</small></article></section>
    ...
    <section class="panel"><h2>「每月判斷」不等於「每月交易」</h2><p>模型每個完整月末都重算狀態，但狀態沒變就不下單。報表把完成成交數與年換手直接列出；最後的 60/40 或 100/0 是政策狀態，不是要求既有持有人每月把漂移權重拉回目標。</p></section>
    <section class="panel"><h2>三個不重疊年代</h2><p class="note">前兩段曾用於舊研究；第三段在協議與資料契約鎖定後才首次下載。Nasdaq Composite 比 Nasdaq-100 更廣，而且代理期都是不含股息的價格指數，因此只能檢驗機制。</p><div class="table-wrap"><table><thead><tr><th>資料段</th><th>策略 / 市場 CAGR</th><th>年化差</th><th>50 bps 年化差</th><th>最大回撤：策略 / 市場</th><th>年換手</th><th>完成交易</th><th>五年勝率</th><th>NW t</th><th>PSR</th></tr></thead><tbody>{period_rows}</tbody></table></div></section>
    ...
    <section class="panel"><h2>搜尋偏誤與前瞻缺口</h2><p>v9 是看過 v8 後才提出，已計入 6,106 次搜尋。DSR：主期 {_pct(dsr["main"]["probability"], 3)}、舊代理 {_pct(dsr["old_proxy"]["probability"], 3)}、全新外部 {_pct(dsr["external"]["probability"], 3)}。即使歷史門檻通過，仍須 252 個新增交易日與 6 次實際完成的狀態切換，且前瞻 NW t、PSR、SPY 超額與回撤全部達標。</p></section>
    <section class="panel"><h2>不可回改的資料收據</h2><p>研究協議 SHA-256：<code>{html.escape(audit["protocol"]["sha256"])}</code><br>外部資料契約 SHA-256：<code>{html.escape(audit["protocol"]["external_data_contract_sha256"])}</code><br>Nasdaq Composite panel：<code>{html.escape(audit["data_receipts"]["external_ixic"]["panel_sha256"])}</code><br>外部 S&amp;P 500 panel：<code>{html.escape(audit["data_receipts"]["external_gspc"]["panel_sha256"])}</code></p></section>
...
def build_hierarchical_defense_report(destination: str | Path, audit: dict[str, Any]) -> Path:
    ...
    period_rows = "".join(
        [
            period_row(
                "可交易 ETF 主期",
                audit["main"],
                "2006-07-31–2026-07-31；SPY／QQQ／SHY 調整後總報酬",
            ),
            period_row(
                "舊 Nasdaq-100 代理",
                audit["old_proxy"],
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font:15px/1.55 ui-sans-serif,system-ui,-apple-system,"PingFang TC","Noto Sans TC",sans-serif}main{max-width:1240px;margin:auto;padding:42px 22px 80px}header{border-bottom:3px solid var(--ink);padding-bottom:24px}.eyebrow{font-weight:800;letter-spacing:.13em;color:var(--gold)}h1{font-size:clamp(34px,7vw,68px);line-height:1.03;margin:8px 0 16px;letter-spacing:-.05em}h2{margin:0 0 14px;font-size:24px}.lead{max-width:940px;font-size:18px}.verdict{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin:24px 0}.card,.panel{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:20px}.card strong{display:block;font-size:30px}.card span,.note,small{display:block;color:var(--muted)}.fail{color:var(--red);font-weight:750}.pass{color:var(--green);font-weight:750}.panel{margin-top:16px}.warning{border-left:5px solid var(--red);background:#f8e7e2;padding:16px 18px;border-radius:8px;margin-top:16px}.success{border-left-color:var(--green);background:#e5f2e9}.table-wrap{overflow:auto}table{width:100%;border-collapse:collapse;min-width:1180px}th,td{text-align:right;border-bottom:1px solid var(--line);padding:11px 9px;white-space:nowrap}th:first-child{text-align:left}thead th{color:var(--muted);font-size:12px}code{font-size:11px;word-break:break-all;white-space:normal}.footer{color:var(--muted);font-size:12px;margin-top:24px}@media(max-width:900px){.verdict{grid-template-columns:1fr 1fr}}@media(max-width:600px){.verdict{grid-template-columns:1fr}main{padding:24px 12px 56px}.panel{padding:14px}}
    ...
    if paper_ok:
        ...
        decision = (
            f"23 道 Paper 入口有 {len(failed_paper)} 道失敗；保留完整負結果，"
            "不建立帳戶，也不回頭調整 60/40、三態順序或門檻。"
        )
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v12 階層式三態研究收據</title><style>{css}</style></head><body><main>
    ...
    <section class="verdict"><article class="card"><span>Paper 入口</span><strong class={"pass" if paper_ok else "fail"}>{audit["paper_entry_passed_gate_count"]} / {audit["paper_entry_required_gate_count"]}</strong><small>三段經濟與資料門檻</small></article><article class="card"><span>全部歷史門檻</span><strong class={"pass" if history_ok else "fail"}>{audit["passed_gate_count"]} / {audit["required_gate_count"]}</strong><small>另含 NW 與 PSR</small></article><article class="card"><span>主期策略 / SPY CAGR</span><strong>{_pct(strategy["cagr"], 2)} / {_pct(spy["cagr"], 2)}</strong><small>50 bps 差 {_pct(main["cost_50bps"]["cagr_difference"], 2)}</small></article><article class="card"><span>最新研究狀態</span><strong>{html.escape(policy)}</strong><small>{"只供新 Paper" if paper_ok else "淘汰研究，不可照單"}</small></article></section>
    ...
    <section class="panel"><h2>新手版三態</h2><p><b>growth：</b>60% 核心＋40% 成長；<b>core：</b>100% 核心；<b>defense：</b>60% 核心＋40% 防守。表中的月數只是每月底判斷結果；完成交易數才是真正換倉次數。兩次換倉間持倉會自然漂移，不要求每月拉回 60/40。</p></section>
    <section class="panel"><h2>三個不重疊年代</h2><p class="note">只有主期是可交易 ETF 總報酬；B/C 是不含股息的價格指數機制測試，防守又刻意設為零利息 CASH，所以不可把三段串成一條實際投資績效。</p><div class="table-wrap"><table><thead><tr><th>資料段</th><th>策略 / 市場 CAGR</th><th>年化差</th><th>50 bps 年化差</th><th>最大回撤：策略 / 市場</th><th>年換手</th><th>完成交易</th><th>月數 G/C/D</th><th>五年勝率</th><th>NW t</th><th>PSR</th></tr></thead><tbody>{period_rows}</tbody></table></div></section>
    <section class="panel"><h2>二十九道凍結歷史門檻</h2><p class="note">Paper 入口通過 {audit["paper_entry_passed_gate_count"]} / {audit["paper_entry_required_gate_count"]}；全部歷史通過 {audit["passed_gate_count"]} / {audit["required_gate_count"]}。任一入口失敗都不開 Paper。</p><div class="table-wrap"><table><thead><tr><th>固定檢查鍵</th><th>結果</th></tr></thead><tbody>{gate_rows}</tbody></table></div></section>
    <section class="panel"><h2>兩次 DJIA 取數失敗沒有被隱藏</h2><p>v10 的 Yahoo 來源沒有 1971–1988 DJIA；v11 事前指定的 S&amp;P DJI 官方 Excel 唯一一次 GET 回覆 403。兩次都在任何策略計算前封存，v12 沒有重試、補值或替換來源，只用既有三段凍結資料第一次計算同一套規則。</p></section>
    <section class="panel"><h2>搜尋偏誤與前瞻缺口</h2><p>DSR 以 6,109 次搜尋計算：主期 {_pct(dsr["main"]["probability"], 3)}、舊代理 {_pct(dsr["old_proxy"]["probability"], 3)}、外部期 {_pct(dsr["external"]["probability"], 3)}。仍須 252 個新增交易日與 6 次實際完成的狀態切換，且前瞻 NW t、PSR、SPY 超額與回撤全部達標，才可評估實金參考。</p></section>
...
def build_report(
    ...
    *{box-sizing:border-box}body{margin:0;background:linear-gradient(145deg,#07101e,#101b2a 48%,#0a1624);color:var(--text);font:15px/1.55 ui-sans-serif,system-ui,-apple-system,"PingFang TC","Noto Sans TC",sans-serif}
    ...
    """
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>US FDDK 美股研究台</title><style>{css}</style></head><body><main>
    <header><div><div class="eyebrow">Evidence-first US market research</div><h1>美股資料與策略研究台</h1><p class="lead">把 20 年行情、訊號、成交、基準與驗證放在同一張可重現的收據上。主策略只使用 ETF 避免把今天的成分股名單誤當成歷史；個股區是當期觀察清單，不是買賣指令。</p><p><a href="paper_volatility.html">開啟成長守門員 v2 LIVE paper →</a>　<a href="paper_growth.html">v1 封存帳戶</a>　<a href="paper_candidate.html">平衡候選帳戶</a></p></div><div class="stamp">研究區間<br><strong>{research_start.date()}–{research_end.date()}</strong><br>資料截止 {latest}</div></header>
    <section class="grid4"><div class="kpi"><span>研究期間</span><strong>{research_years:.1f} 年</strong></div><div class="kpi"><span>SPY 相對 200 日線</span><strong class="{"good" if spy_trend >= 0 else "bad"}">{_pct(spy_trend)}</strong></div><div class="kpi"><span>VIX</span><strong>{_num(vix_last, 1)}</strong></div><div class="kpi"><span>資料契約</span><strong class="{"good" if contract.ok else "bad"}">{"通過" if contract.ok else "未通過"}</strong></div></section>
    <section class="panel"><div class="panel-head"><div><h2>ETF 主線：同一時鐘、同一成本</h2><p>月末收盤產生訊號，下一交易日開盤成交；含雙邊換手成本。表內 Sharpe 是零利率口徑，下一節另列扣除 SHY 後的標準超額口徑。</p></div></div>{chart}<div class="table-wrap"><table><thead><tr><th>策略</th><th>累積報酬</th><th>CAGR</th><th>波動</th><th>Sharpe（0%）</th><th>Sortino</th><th>最大回撤</th><th>Calmar</th><th>年換手</th><th>換倉次數</th></tr></thead><tbody>{_metrics_table(headline_results)}</tbody></table></div></section>
    ...
    <section class="panel"><div class="panel-head"><h2>v3 三層硬門檻</h2><p>QQQ、固定 96/4 曝險控制與 1986–2006 代理期分開判讀；代理期任何硬門檻失敗就不升級。</p></div><div class="table-wrap"><table><thead><tr><th>層級</th><th>檢查</th><th>結果</th></tr></thead><tbody>{_v3_gate_rows(validations)}</tbody></table></div></section>
    ...
    <section class="panel"><div class="panel-head"><div><h2>Paper-only 風險管理候選：成長守門員 v2</h2><p>以無槓桿波動管理降低 QQQ 回撤；新增曝險匹配基準後，不能再只用勝過 SPY 當作 alpha 證據。</p></div></div>{_volatility_guard_evidence(validations)}</section>
    ...
    <section class="panel"><div class="panel-head"><h2>新增的曝險控制稽核：被動 90% QQQ／10% SHY</h2><p>這組稽核是在 v2 選定後補上，因此不能冒充預先註冊；它用簡單月末再平衡組合回答「波動管理是否真的優於相近 QQQ 曝險」。任一一致性門檻失敗，就只保留 Paper，不升級成參考下單策略。</p></div><div class="table-wrap"><table><thead><tr><th>檢查</th><th>結果</th></tr></thead><tbody>{_exposure_control_gate_rows(validations)}</tbody></table></div></section>
    <section class="panel"><div class="panel-head"><h2>v2 波動窗／目標波動鄰域</h2><p>只有 3×3 共九組。18% 是中間風險政策；22% 報酬更高、14% 回撤更低，故 18% 不是端點贏家。</p></div><div class="table-wrap"><table><thead><tr><th>波動窗</th><th>目標波動</th><th>角色</th><th>CAGR</th><th>Sharpe</th><th>MDD</th><th>相對 SPY CAGR</th></tr></thead><tbody>{_volatility_family_rows(validations)}</tbody></table></div></section>
    ...
    <section class="panel"><div class="panel-head"><div><h2>v1 封存比較：永久 80% QQQ＋趨勢核心</h2><p>v1 仍完整保留，但因 v2 同時提高 CAGR 並降低最大回撤，不再作網站主訊號。</p></div></div>{_growth_guard_evidence(validations)}</section>
    ...
    <section class="panel"><div class="panel-head"><h2>v1 成長／防守比例鄰域</h2><p>80/20 是當時的風險政策上限，不是鄰域中最高報酬的一列。</p></div><div class="table-wrap"><table><thead><tr><th>趨勢核心</th><th>QQQ 衛星</th><th>角色</th><th>CAGR</th><th>Sharpe</th><th>MDD</th><th>相對 SPY CAGR</th></tr></thead><tbody>{_growth_family_rows(validations)}</tbody></table></div></section>
    ...
    <section class="panel"><div class="panel-head"><div><h2>較低風險研究線：Sharpe &gt; 1 關卡</h2><p>另一候選是 75% 多資產趨勢／風險核心＋25% QQQ；SHY 是可交易的短債／無風險代理，必須從報酬中扣除。</p></div></div>{_candidate_evidence(validations)}</section>
    <section class="panel"><div class="panel-head"><h2>候選的參數鄰域</h2><p>只改核心比例；同時列出零利率與 SHY 超額 Sharpe，避免低波動短債把結果灌高。</p></div><div class="table-wrap"><table><thead><tr><th>核心比例</th><th>角色</th><th>CAGR</th><th>Sharpe（0%）</th><th>Sharpe（超額）</th><th>MDD</th><th>年換手</th></tr></thead><tbody>{_candidate_neighborhood_rows(validations)}</tbody></table></div></section>
    ...
    <section class="panel"><div class="panel-head"><h2>與基準的成對檢驗</h2><p>Newey–West 修正自相關；|t| &lt; 1.96 不作「勝過基準」宣稱。</p></div><div class="table-wrap"><table><thead><tr><th>比較</th><th>CAGR 差</th><th>年化平均主動報酬</th><th>NW t</th><th>判讀</th></tr></thead><tbody>{_validation_rows(validations)}</tbody></table></div></section>
    ...
    <section class="panel"><div class="panel-head"><h2>前後十年分段</h2><p>每格依序是 CAGR／Sharpe／最大回撤；檢查結論是否只由單一時代支撐。</p></div><div class="table-wrap"><table><thead><tr><th>期間</th>{"".join(f"<th>{html.escape(result.name)}</th>" for result in headline_results)}</tr></thead><tbody>{_subperiod_rows(validations, headline_results)}</tbody></table></div></section>
    <section class="panel"><div class="panel-head"><h2>三次壓力期</h2><p>每格依序是區間報酬／區間內最大回撤；日期在研究前固定，不按結果挑選。</p></div><div class="table-wrap"><table><thead><tr><th>壓力期</th>{"".join(f"<th>{html.escape(result.name)}</th>" for result in headline_results)}</tr></thead><tbody>{_stress_rows(validations, headline_results)}</tbody></table></div></section>
    ...
    <section class="panel"><div class="panel-head"><h2>原雙動量參數與成本鄰域</h2><p>完整列出鄰域，不挑事後最高報酬；原設定為 252 日、Top 4、10 bps，保留作候選策略以外的基準與負結果。</p></div><div class="table-wrap"><table><thead><tr><th>回顧日</th><th>Top K</th><th>成本 bps</th><th>CAGR</th><th>Sharpe</th><th>MDD</th></tr></thead><tbody>{_sensitivity_rows(validations)}</tbody></table></div></section>
    <section class="panel"><div class="panel-head"><div><h2>大型股當期觀察</h2><p>12–1 月動量 45%＋6–1 月動量 25%＋200 日趨勢 20%＋低波動 10%。名單是 {html.escape(str(stock_screen["universe_as_of"].iloc[0] if len(stock_screen) else "—"))} 快照，因此不拿它做無偏歷史績效宣稱。</p></div></div><div class="table-wrap"><table><thead><tr><th>代號</th><th>產業</th><th>分數</th><th>12–1 月</th><th>距 200MA</th><th>年化波動</th></tr></thead><tbody>{_screen_rows(stock_screen)}</tbody></table></div></section>
    <section class="panel"><h2>個股傾斜診斷（有生存者偏誤）</h2><div class="warn">這一區只回答「在今天這批大型股內，廣泛持有後再做權重傾斜，是否值得繼續研究」。它不能證明歷史可交易，因為缺少逐期成分股與下市股票。</div><div class="table-wrap"><table><thead><tr><th>策略</th><th>累積報酬</th><th>CAGR</th><th>波動</th><th>Sharpe</th><th>Sortino</th><th>最大回撤</th><th>Calmar</th><th>年換手</th><th>換倉次數</th></tr></thead><tbody>{_metrics_table(stock_results)}</tbody></table></div></section>
    <section class="panel"><h2>設計邊界</h2><div class="notes"><article><h3>我們採用</h3><p>凍結原始快照、調整後 OHLC 一致、前一日訊號、成本敏感度、廣泛持有後傾斜、SPY／等權池雙基準、負結果與不顯著結果照樣呈現。</p></article><article><h3>我們不假裝</h3><p>FINRA 場外資料有彙總與發布延遲，不能反推即時「大戶買進」；SEC 財報是申報日後才可用；今天的 S&amp;P 500 持股不是過去的成分股；免費行情可能回溯修訂。</p></article></div></section>
    <section class="panel"><h2>資料收據</h2><p class="{"ok" if contract.ok else "warn"}">{"資料契約通過。" if contract.ok else "資料契約未通過，結果不得發布。"} {html.escape("；".join(contract.warnings))}</p><p>供應商：{html.escape(str(panel.metadata.get("provider", "—")))}<br>還原方式：{html.escape(str(panel.metadata.get("adjustment", "—")))}<br>快照 SHA-256：<span class="hash">{html.escape(str(archive_hash))}</span></p></section>
...
def build_confirmed_relative_growth_report(destination: str | Path, audit: dict[str, Any]) -> Path:
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font:15px/1.55 ui-sans-serif,system-ui,-apple-system,"PingFang TC","Noto Sans TC",sans-serif}main{max-width:1240px;margin:auto;padding:42px 22px 80px}header{border-bottom:3px solid var(--ink);padding-bottom:24px}.eyebrow{font-weight:800;letter-spacing:.13em;color:var(--gold)}h1{font-size:clamp(34px,7vw,68px);line-height:1.03;margin:8px 0 16px;letter-spacing:-.05em}h2{margin:0 0 14px;font-size:24px}.lead{max-width:940px;font-size:18px}.verdict{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin:24px 0}.card,.panel{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:20px}.card strong{display:block;font-size:30px}.card span,.note,small{display:block;color:var(--muted)}.fail{color:var(--red);font-weight:750}.pass{color:var(--green);font-weight:750}.panel{margin-top:16px}.warning{border-left:5px solid var(--red);background:#f8e7e2;padding:16px 18px;border-radius:8px;margin-top:16px}.success{border-left-color:var(--green);background:#e5f2e9}.table-wrap{overflow:auto}table{width:100%;border-collapse:collapse;min-width:1120px}th,td{text-align:right;border-bottom:1px solid var(--line);padding:11px 9px;white-space:nowrap}th:first-child{text-align:left}thead th{color:var(--muted);font-size:12px}code{font-size:11px;word-break:break-all;white-space:normal}a{color:var(--green)}.footer{color:var(--muted);font-size:12px;margin-top:24px}@media(max-width:900px){.verdict{grid-template-columns:1fr 1fr}}@media(max-width:600px){.verdict{grid-template-columns:1fr}main{padding:24px 12px 56px}.panel{padding:14px}}
    ...
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v13 兩月確認相對成長新 ETF 驗證</title><style>{css}</style></head><body><main>
    ...
    <section class="verdict"><article class="card"><span>新資料經濟門檻</span><strong class={"pass" if audit["economic_passed_gate_count"] == audit["economic_required_gate_count"] else "fail"}>{audit["economic_passed_gate_count"]} / {audit["economic_required_gate_count"]}</strong><small>三組各十道，全過才算</small></article><article class="card"><span>資料與暖機</span><strong class={"pass" if audit["data_passed_gate_count"] == audit["data_required_gate_count"] else "fail"}>{audit["data_passed_gate_count"]} / {audit["data_required_gate_count"]}</strong><small>固定起點不可事後移動</small></article><article class="card"><span>歷史統計門檻</span><strong class={"pass" if audit["statistical_passed_gate_count"] == audit["statistical_required_gate_count"] else "fail"}>{audit["statistical_passed_gate_count"]} / {audit["statistical_required_gate_count"]}</strong><small>NW、PSR、DSR</small></article><article class="card"><span>Paper 決定</span><strong class={"pass" if paper_ok else "fail"}>{"可建立" if paper_ok else "不建立"}</strong><small>不能顯示下單配置</small></article></section>
    <div class="warning {"success" if paper_ok else ""}"><b>結論：{"只准新建隔離 Paper。" if paper_ok else "新樣本否決候選。"}</b><br>{decision}</div>
    <section class="panel"><h2>新手先看這裡</h2><p>「回撤較小」不等於「策略有超額報酬」。Russell 1000 組雖把最大回撤從 {_pct(datasets["russell_1000"]["benchmark_metrics"]["market"]["max_drawdown"], 1)} 降到 {_pct(datasets["russell_1000"]["strategy_metrics"]["max_drawdown"], 1)}，20 年 CAGR 卻由 {_pct(datasets["russell_1000"]["benchmark_metrics"]["market"]["cagr"], 2)} 降為 {_pct(datasets["russell_1000"]["strategy_metrics"]["cagr"], 2)}；Russell 2000 也落後。若只挑「比較不痛」而忽略「長期少賺」，就會把風險控制誤當成 alpha。</p></section>
    <section class="panel"><h2>三組下載後才第一次計算的 ETF</h2><div class="table-wrap"><table><thead><tr><th>驗證組</th><th>門檻</th><th>暖機</th><th>策略 / 核心 CAGR</th><th>10 bps 年化差</th><th>50 bps 年化差</th><th>最大回撤：策略 / 核心</th><th>相對同狀態對照</th><th>五年勝率</th><th>NW t</th></tr></thead><tbody>{dataset_rows}</tbody></table></div></section>
    <section class="panel"><h2>為什麼這比再調參重要</h2><p>既有三年代讓兩月確認版本看起來有希望；但那些資料已參與提出規則，不能再當獨立證據。新的大中型、小型與海外 ETF 是在協議凍結後才下載，結果直接反駁「能跨股票母體穩健跑贏」的說法。正確動作是保留負結果，而不是把 40/60、70/30 或兩個月改成另一個看起來較漂亮的數字。</p></section>
    <section class="panel"><h2>三十道新資料經濟門檻</h2><div class="table-wrap"><table><thead><tr><th>固定檢查鍵</th><th>結果</th></tr></thead><tbody>{economic_rows}</tbody></table></div></section>
    <section class="panel"><h2>資料與暖機門檻</h2><div class="table-wrap"><table><thead><tr><th>固定檢查鍵</th><th>結果</th></tr></thead><tbody>{data_rows}</tbody></table></div></section>
    <section class="panel"><h2>標的與不可回改收據</h2><p><a href="https://www.ishares.com/us/products/239706/">IWF</a> 追蹤 Russell 1000 Growth Index；<a href="https://www.ishares.com/us/products/239707/">IWB</a> 追蹤 Russell 1000 Index。其餘兩組同樣以發行人定義與凍結調整後行情驗證。協議 SHA-256：<code>{html.escape(audit["protocol"]["sha256"])}</code><br>新 ETF 面板 SHA-256：<code>{html.escape(audit["snapshot"]["panel_sha256"])}</code></p></section>
    <p class="footer">歷史回測、研究與教育用途，不構成投資建議。淘汰研究的最後政策狀態不是下單訊號。</p>
    </main></body></html>"""
    destination.write_text(content, encoding="utf-8")
    return destination
...
def build_modest_leverage_report(destination: str | Path, audit: dict[str, Any]) -> Path:
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font:15px/1.55 ui-sans-serif,system-ui,-apple-system,"PingFang TC","Noto Sans TC",sans-serif}main{max-width:1240px;margin:auto;padding:42px 22px 80px}header{border-bottom:3px solid var(--ink);padding-bottom:24px}.eyebrow{font-weight:800;letter-spacing:.13em;color:var(--gold)}h1{font-size:clamp(34px,7vw,68px);line-height:1.03;margin:8px 0 16px;letter-spacing:-.05em}h2{margin:0 0 14px;font-size:24px}.lead{max-width:940px;font-size:18px}.verdict{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin:24px 0}.card,.panel{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:20px}.card strong{display:block;font-size:30px}.card span,.note,small{display:block;color:var(--muted)}.fail{color:var(--red);font-weight:750}.pass{color:var(--green);font-weight:750}.panel{margin-top:16px}.warning{border-left:5px solid var(--red);background:#f8e7e2;padding:16px 18px;border-radius:8px;margin-top:16px}.success{border-left-color:var(--green);background:#e5f2e9}.table-wrap{overflow:auto}table{width:100%;border-collapse:collapse;min-width:1120px}th,td{text-align:right;border-bottom:1px solid var(--line);padding:11px 9px;white-space:nowrap}th:first-child{text-align:left}thead th{color:var(--muted);font-size:12px}code{font-size:11px;word-break:break-all;white-space:normal}a{color:var(--green)}.footer{color:var(--muted);font-size:12px;margin-top:24px}@media(max-width:900px){.verdict{grid-template-columns:1fr 1fr}}@media(max-width:600px){.verdict{grid-template-columns:1fr}main{padding:24px 12px 56px}.panel{padding:14px}}
    ...
    decision = (
        "36 道經濟門檻與所有資料門檻全過；仍須統計與前瞻期。"
        if paper_ok
        else (
            "S&P 500 與 Dow 30 的長期報酬落後原始 ETF；Nasdaq-100 雖略勝 QQQ，"
            "仍落後不擇時的同產品 60/40。依預先協議封存，不調比例、不建立 v14 Paper。"
        )
    )
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v14 小幅槓桿趨勢驗證</title><style>{css}</style></head><body><main>
    <header><div class="eyebrow">PRE-REGISTERED LEVERAGED-ETF VALIDATION</div><h1>{headline}</h1><p class="lead">v14 只做一件事：市場連續兩個完整月高於 200 日均線時，持有 60% 實際 2 倍每日目標 ETF／40% SHY；確認轉弱時改為 100% SHY。比例、成本、三組代號與 36 道門檻先寫死，之後才下載 SSO、QLD、DDM。</p></header>
    <section class="verdict"><article class="card"><span>經濟門檻</span><strong class={"pass" if audit["economic_passed_gate_count"] == audit["economic_required_gate_count"] else "fail"}>{audit["economic_passed_gate_count"]} / {audit["economic_required_gate_count"]}</strong><small>三組各 12 道，任一失敗即淘汰</small></article><article class="card"><span>資料與時序</span><strong class={"pass" if audit["data_passed_gate_count"] == audit["data_required_gate_count"] else "fail"}>{audit["data_passed_gate_count"]} / {audit["data_required_gate_count"]}</strong><small>先凍結，再下載與計算</small></article><article class="card"><span>統計門檻</span><strong class={"pass" if audit["statistical_passed_gate_count"] == audit["statistical_required_gate_count"] else "fail"}>{audit["statistical_passed_gate_count"]} / {audit["statistical_required_gate_count"]}</strong><small>相對原始 ETF 與固定 60/40</small></article><article class="card"><span>Paper 決定</span><strong class={"pass" if paper_ok else "fail"}>{"可建立" if paper_ok else "不建立"}</strong><small>失敗候選不顯示下單比例</small></article></section>
    <div class="warning {"success" if paper_ok else ""}"><b>結論：{"只准開始隔離 Paper。" if paper_ok else "新資料否決候選。"}</b><br>{decision}</div>
    <section class="panel"><h2>新手先看：槓桿不是免費報酬</h2><p>2 倍 ETF 追求的是「每日」兩倍，不是二十年報酬一定兩倍。每日重設、費用、波動與複利都會改變長期結果。即使只把 60% 放入 2 倍 ETF，風險開啟時仍約有 120% 名目股票曝險，可能比普通 ETF 跌得更多；所以本研究直接使用真實基金價格，並要求同時比較原始 ETF 與固定 60/40。</p></section>
    <section class="panel"><h2>20 年三市場結果</h2><div class="table-wrap"><table><thead><tr><th>市場</th><th>門檻</th><th>策略 CAGR</th><th>原始 ETF</th><th>固定 60/40</th><th>相對原始</th><th>相對 60/40</th><th>回撤：策略 / 原始</th><th>五年勝率</th><th>NW t</th></tr></thead><tbody>{dataset_rows}</tbody></table></div></section>
    ...
    <section class="panel"><h2>資料、時序與權重門檻</h2><div class="table-wrap"><table><thead><tr><th>固定檢查鍵</th><th>結果</th></tr></thead><tbody>{data_rows}</tbody></table></div></section>
...
def build_modest_leverage_overlay_report(destination: str | Path, audit: dict[str, Any]) -> Path:
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font:15px/1.55 ui-sans-serif,system-ui,-apple-system,"PingFang TC","Noto Sans TC",sans-serif}main{max-width:1240px;margin:auto;padding:42px 22px 80px}header{border-bottom:3px solid var(--ink);padding-bottom:24px}.eyebrow{font-weight:800;letter-spacing:.13em;color:var(--gold)}h1{font-size:clamp(34px,7vw,68px);line-height:1.03;margin:8px 0 16px;letter-spacing:-.05em}h2{margin:0 0 14px;font-size:24px}.lead{max-width:940px;font-size:18px}.verdict{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin:24px 0}.card,.panel{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:20px}.card strong{display:block;font-size:30px}.card span,.note,small{display:block;color:var(--muted)}.fail{color:var(--red);font-weight:750}.pass{color:var(--green);font-weight:750}.panel{margin-top:16px}.warning{border-left:5px solid var(--red);background:#f8e7e2;padding:16px 18px;border-radius:8px;margin-top:16px}.success{border-left-color:var(--green);background:#e5f2e9}.table-wrap{overflow:auto}table{width:100%;border-collapse:collapse;min-width:1180px}th,td{text-align:right;border-bottom:1px solid var(--line);padding:11px 9px;white-space:nowrap}th:first-child{text-align:left}thead th{color:var(--muted);font-size:12px}code{font-size:11px;word-break:break-all;white-space:normal}a{color:var(--green)}.footer{color:var(--muted);font-size:12px;margin-top:24px}@media(max-width:900px){.verdict{grid-template-columns:1fr 1fr}}@media(max-width:600px){.verdict{grid-template-columns:1fr}main{padding:24px 12px 56px}.panel{padding:14px}}
    ...
    headline = (
        ...
        if paper_ok
        else "報酬確實較高，<br>但風險證據不合格。"
    )
    decision = (
        "36 道經濟門檻與資料門檻全過；仍不得當成實金訊號。"
        if paper_ok
        else (
            "三組 CAGR 都高於原始 ETF，但 S&P 500、Nasdaq-100、Dow 30 的最大回撤"
            "也全部更深，三組 Sharpe 都沒有嚴格勝過原始 ETF。依凍結門檻不建 Paper。"
        )
    )
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v15 小幅槓桿疊加驗證</title><style>{css}</style></head><body><main>
    <header><div class="eyebrow">FIRST-SEEN 3X ETF VALIDATION</div><h1>{headline}</h1><p class="lead">v15 平時持有原始 ETF；只有連續兩個完整月站上 200 日均線時，才改為 90% 原始 ETF／10% 實際每日 3 倍 ETF，把名目股票曝險提高到約 120%。規則與 36 道門檻先凍結，之後才首次下載 UPRO、TQQQ、UDOW。</p></header>
    <section class="verdict"><article class="card"><span>經濟門檻</span><strong class={"pass" if audit["economic_passed_gate_count"] == audit["economic_required_gate_count"] else "fail"}>{audit["economic_passed_gate_count"]} / {audit["economic_required_gate_count"]}</strong><small>三市場各 12 道</small></article><article class="card"><span>資料與時序</span><strong class={"pass" if audit["data_passed_gate_count"] == audit["data_required_gate_count"] else "fail"}>{audit["data_passed_gate_count"]} / {audit["data_required_gate_count"]}</strong><small>先凍結，再下載與計算</small></article><article class="card"><span>統計門檻</span><strong class={"pass" if audit["statistical_passed_gate_count"] == audit["statistical_required_gate_count"] else "fail"}>{audit["statistical_passed_gate_count"]} / {audit["statistical_required_gate_count"]}</strong><small>相對原始 ETF 與固定 90/10</small></article><article class="card"><span>Paper 決定</span><strong class={"pass" if paper_ok else "fail"}>{"可建立" if paper_ok else "不建立"}</strong><small>不合格就不顯示比例</small></article></section>
    <div class="warning {"success" if paper_ok else ""}"><b>結論：{"只准開始隔離 Paper。" if paper_ok else "較高報酬不等於穩健跑贏。"}</b><br>{decision}</div>
    <section class="panel"><h2>新手先看：為什麼「三組都賺比較多」仍然不合格？</h2><p>加上槓桿本來就可能把多頭市場的報酬放大，但使用者同時承受更大的虧損。這次 S&P 500 CAGR {_pct(sp500["strategy_metrics"]["cagr"], 2)} 對 SPY {_pct(sp500["benchmark_metrics"]["core"]["cagr"], 2)}；Nasdaq-100 {_pct(nasdaq["strategy_metrics"]["cagr"], 2)} 對 QQQ {_pct(nasdaq["benchmark_metrics"]["core"]["cagr"], 2)}；Dow 30 {_pct(dow["strategy_metrics"]["cagr"], 2)} 對 DIA {_pct(dow["benchmark_metrics"]["core"]["cagr"], 2)}。但三組最大回撤全都更深，不能只挑報酬欄宣布成功。</p></section>
    <section class="panel"><h2>15 年首次查看的三市場結果</h2><div class="table-wrap"><table><thead><tr><th>市場</th><th>門檻</th><th>策略 CAGR</th><th>原始 ETF</th><th>固定 90/10</th><th>Sharpe：策略 / ETF</th><th>回撤：策略 / ETF</th><th>相對 ETF</th><th>相對固定</th><th>五年勝率</th><th>NW t</th></tr></thead><tbody>{"".join(dataset_rows)}</tbody></table></div></section>
    <section class="panel"><h2>20 年與 15 年不能混寫</h2><p>v14 的 20 年 2 倍 ETF 資料只用來產生這個架構；v15 的確認證據是規則凍結後才看的 3 倍 ETF 實際價格，受產品成立日限制只有 15 年。報告不把兩段拼成「獨立 20 年」。</p></section>
    <section class="panel"><h2>36 道預先固定的經濟門檻</h2><div class="table-wrap"><table><thead><tr><th>固定檢查鍵</th><th>結果</th></tr></thead><tbody>{economic_rows}</tbody></table></div></section>
    <section class="panel"><h2>資料、時序與權重門檻</h2><div class="table-wrap"><table><thead><tr><th>固定檢查鍵</th><th>結果</th></tr></thead><tbody>{data_rows}</tbody></table></div></section>
...
def build_trend_volatility_brake_report(destination: str | Path, audit: dict[str, Any]) -> Path:
    ...
    decision = (
        "經濟與資料入口全數通過；仍需統計與 252 個新交易日。"
        if paper_ok
        else "三組都只通過 2/16。退場與週週微調降低了部分回撤，卻大幅犧牲長期報酬；依凍結規則封存，不調目標波動、不建立 v16 Paper。"
    )
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v16 趨勢與波動煞車驗證</title><style>{css}</style></head><body><main>
    <header><div class="eyebrow">PRE-REGISTERED WEEKLY RISK BRAKE</div><h1>{headline}</h1><p class="lead">v16 在核心 ETF 高於 200 日均線時，依 21 日實現波動把股票名目曝險固定限制在約 100%–150%；趨勢轉弱時全部進 SHY。訊號只用已完成交易週，下一交易日開盤執行。</p></header>
    <section class="verdict"><article class="card"><span>經濟門檻</span><strong class={"pass" if audit["economic_passed_gate_count"] == audit["economic_required_gate_count"] else "fail"}>{audit["economic_passed_gate_count"]} / {audit["economic_required_gate_count"]}</strong><small>三組各 16 道</small></article><article class="card"><span>資料與時序</span><strong class={"pass" if audit["data_passed_gate_count"] == audit["data_required_gate_count"] else "fail"}>{audit["data_passed_gate_count"]} / {audit["data_required_gate_count"]}</strong><small>新 ETF、週末與權重</small></article><article class="card"><span>統計門檻</span><strong class={"pass" if audit["statistical_passed_gate_count"] == audit["statistical_required_gate_count"] else "fail"}>{audit["statistical_passed_gate_count"]} / {audit["statistical_required_gate_count"]}</strong><small>三個公平基準</small></article><article class="card"><span>Paper 決定</span><strong class={"pass" if paper_ok else "fail"}>{"可建立" if paper_ok else "不建立"}</strong><small>淘汰版不顯示配置</small></article></section>
    <div class="warning {"success" if paper_ok else ""}"><b>結論：{"只准隔離 Paper。" if paper_ok else "歷史樣本否決候選。"}</b><br>{decision}</div>
    <section class="panel"><h2>新手先看：少跌一點，可能也少賺很多</h2><p>這個版本不是因為程式壞掉而失敗。它確實在三組市場把最大回撤壓在約 38%–44%，但年化報酬只剩約 2.8%–5.6%，低於原始 ETF，也低於不加槓桿的相同趨勢控制。週度調整與整體退場造成的踏空，是這次最重要的負面證據。</p></section>
    <section class="panel"><h2>18 年三市場結果</h2><div class="table-wrap"><table><thead><tr><th>市場</th><th>門檻</th><th>策略 CAGR</th><th>原始 ETF</th><th>不槓桿同趨勢</th><th>固定 150%</th><th>Sharpe：策略 / ETF</th><th>回撤：策略 / ETF</th><th>週訊號</th><th>實際換倉</th><th>NW t</th></tr></thead><tbody>{"".join(rows)}</tbody></table></div></section>
    <section class="panel"><h2>為何不是 20 年</h2><p>MVV、UWM、SAA 的實際上市歷史不足完整 20 年，因此本報告保留 2008-07-31 至 2026-07-31 的 18 年正式期；沒有用合成的槓桿報酬補齊缺口。</p></section>
    ...
    <section class="panel"><h2>資料與時序門檻</h2><div class="table-wrap"><table><thead><tr><th>檢查鍵</th><th>結果</th></tr></thead><tbody>{_research_gate_rows(audit["data_gates"])}</tbody></table></div></section>
    <section class="panel"><h2>研究依據與收據</h2><p><a href="https://papers.ssrn.com/sol3/papers.cfm?abstract_id=2741701">Leverage for the Long Run</a> 提供長期趨勢槓桿假說；<a href="https://www.nber.org/papers/w22208">Volatility Managed Portfolios</a> 提供反波動曝險的研究動機。它們是待驗證假說，不是本結果保證。協議 SHA-256：<code>{html.escape(audit["protocol"]["sha256"])}</code><br>面板 SHA-256：<code>{html.escape(audit["snapshot"]["panel_sha256"])}</code></p></section>
...
def build_capital_efficient_report(destination: str | Path, audit: dict[str, Any]) -> Path:
    ...
    decision = (
        "84 道經濟與所有資料入口通過；仍需完整統計與前瞻驗證。"
        if paper_ok
        else "六組大多提高 CAGR，也勝過把 40% 留在 SHY 的對照；但相對原始 ETF 的回撤更深，Sharpe 與 Calmar 多數更差。依凍結協議淘汰，不調股債比例、不建立 v17 Paper。"
    )
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v17 資本效率股債組合驗證</title><style>{css}</style></head><body><main>
    <header><div class="eyebrow">FROZEN CAPITAL-EFFICIENT TEST</div><h1>{headline}</h1><p class="lead">v17 每月固定持有 60% 實際每日 2 倍股票 ETF 與 40% IEF，約為 120% 股票加 40% 7–10 年美國公債曝險。沒有均線、預測、停損或事後挑比例。</p></header>
    <section class="verdict"><article class="card"><span>經濟門檻</span><strong class={"pass" if audit["economic_passed_gate_count"] == audit["economic_required_gate_count"] else "fail"}>{audit["economic_passed_gate_count"]} / {audit["economic_required_gate_count"]}</strong><small>六組各 14 道</small></article><article class="card"><span>資料與時序</span><strong class={"pass" if audit["data_passed_gate_count"] == audit["data_required_gate_count"] else "fail"}>{audit["data_passed_gate_count"]} / {audit["data_required_gate_count"]}</strong><small>快照、暖機、月末、權重</small></article><article class="card"><span>統計門檻</span><strong class={"pass" if audit["statistical_passed_gate_count"] == audit["statistical_required_gate_count"] else "fail"}>{audit["statistical_passed_gate_count"]} / {audit["statistical_required_gate_count"]}</strong><small>三基準、NW／PSR／DSR</small></article><article class="card"><span>Paper 決定</span><strong class={"pass" if paper_ok else "fail"}>{"可建立" if paper_ok else "不建立"}</strong><small>失敗即拒絕建立帳戶</small></article></section>
    <div class="warning {"success" if paper_ok else ""}"><b>結論：{"只准隔離 Paper。" if paper_ok else "不是可參考交易策略。"}</b><br>{decision}</div>
    <section class="panel"><h2>新手先看：報酬變高，不代表風險報酬變好</h2><p>這個組合的總名目曝險約 160%。在六組裡，CAGR 多數比原始 ETF 高約 0.1–1.8 個百分點；可是最大回撤約為 −58% 至 −63%，比原始 ETF 更深。也就是說，加入 IEF 比留在短債好一些，卻沒有把槓桿股票的尾部風險降到可接受門檻。</p></section>
    <section class="panel"><h2>六市場長期結果</h2><div class="table-wrap"><table><thead><tr><th>市場</th><th>期間</th><th>門檻</th><th>策略 CAGR</th><th>原始 ETF</th><th>未槓桿 75/25</th><th>2x/SHY</th><th>Sharpe：策略 / ETF</th><th>回撤：策略 / ETF</th><th>Calmar：策略 / ETF</th><th>NW t</th></tr></thead><tbody>{"".join(rows)}</tbody></table></div></section>
    ...
    <section class="panel"><h2>三個公平基準</h2><p>除了原始 1 倍 ETF，本版同時比較未槓桿的 75% 股票／25% IEF，以及相同約 120% 股票曝險但把 40% 放在 SHY 的版本。這能分開回答：超額報酬是否只來自加槓桿，以及中期公債是否真的改善結果。</p></section>
...
def build_equal_diversifier_report(destination: str | Path, audit: dict[str, Any]) -> Path:
    ...
    decision = (
        "18 道外部經濟與所有資料門檻通過；仍需歷史統計與 252 個新交易日。"
        if paper_ok
        else "凍結後的海外日線只通過 5/18 道經濟門檻。已開發市場的 Sharpe、回撤、Calmar、前半期與滾動五年失敗；新興市場更只有 Sharpe 一項通過。依協議不建立 v18 Paper。"
    )
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v18 等權股債金外部驗證</title><style>{css}</style></head><body><main>
    <header><div class="eyebrow">FROZEN STOCK / BOND / GOLD VALIDATION</div><h1>{headline}</h1><p class="lead">v18 每月固定持有 50% 實際每日 2 倍股票 ETF、25% IEF 與 25% GLD，約為 100% 股票、25% 中期美債、25% 黃金曝險。美國六市場只用來選定規則；Paper 決定由凍結後的 EFA/EFO 與 EEM/EET 路徑負責。</p></header>
    <section class="verdict"><article class="card"><span>海外經濟門檻</span><strong class={"pass" if audit["economic_passed_gate_count"] == audit["economic_required_gate_count"] else "fail"}>{audit["economic_passed_gate_count"]} / {audit["economic_required_gate_count"]}</strong><small>兩組各 9 道</small></article><article class="card"><span>資料與時序</span><strong class={"pass" if audit["data_passed_gate_count"] == audit["data_required_gate_count"] else "fail"}>{audit["data_passed_gate_count"]} / {audit["data_required_gate_count"]}</strong><small>先凍結，再下載完整日線</small></article><article class="card"><span>統計門檻</span><strong class={"pass" if audit["statistical_passed_gate_count"] == audit["statistical_required_gate_count"] else "fail"}>{audit["statistical_passed_gate_count"]} / {audit["statistical_required_gate_count"]}</strong><small>核心與同資產不槓桿</small></article><article class="card"><span>Paper 決定</span><strong class={"pass" if paper_ok else "fail"}>{"可建立" if paper_ok else "不建立"}</strong><small>失敗候選不顯示美股配置</small></article></section>
    <div class="warning {"success" if paper_ok else ""}"><b>結論：{"只准隔離 Paper。" if paper_ok else "海外結果否決候選。"}</b><br>{decision}</div>
    <section class="panel"><h2>新手先看：漂亮的 20 年美國回測仍可能過度貼合</h2><p>50/25/25 在六個已見美國市場都提高完整期 CAGR、Sharpe 與 Calmar，也略減回撤；但換到美國以外的 16 年實際 ETF 路徑後，改善沒有穩定重現。這正是外部驗證存在的理由：不是再找一組能過的市場，而是讓先鎖定的規則接受可能被否決的測試。</p></section>
    <section class="panel"><h2>16 年兩組海外結果</h2><div class="table-wrap"><table><thead><tr><th>市場</th><th>門檻</th><th>策略 CAGR</th><th>核心 ETF</th><th>同資產不槓桿</th><th>Sharpe：策略 / 核心</th><th>回撤：策略 / 核心</th><th>Calmar：策略 / 核心</th><th>前 / 後半期 CAGR 差</th><th>五年勝率</th><th>NW t</th></tr></thead><tbody>{"".join(rows)}</tbody></table></div></section>
    <section class="panel"><h2>失敗位置</h2><p>美國以外已開發市場的策略 CAGR 為 8.00%，略高於 EFA 的 7.70%，但 Sharpe 0.490 低於 0.504，最大回撤 −36.2% 深於 −34.2%，前半期 CAGR 又落後約 0.57 個百分點，滾動五年勝率只有 34.8%。新興市場策略 CAGR 5.12%，僅略高於 EEM 的 4.98%，卻低於同資產不槓桿組合的 5.38%；最大回撤從 −39.8% 加深至 −46.7%，滾動五年勝率只有 31.1%。</p></section>
    <section class="panel"><h2>為何不是海外 20 年</h2><p>EFO 與 EET 都在 2009 年才成立，所以正式期固定為 2010-07-30 至 2026-07-31。沒有用合成槓桿報酬補出不存在的產品歷史。20 年只屬於已見的美國大型股設計樣本，不能與本次外部證據混稱。</p></section>
    ...
    <section class="panel"><h2>資料、月末與執行門檻</h2><div class="table-wrap"><table><thead><tr><th>檢查鍵</th><th>結果</th></tr></thead><tbody>{_research_gate_rows(audit["data_gates"])}</tbody></table></div></section>
    <section class="panel"><h2>證據邊界與不可回改收據</h2><p>凍結前已看過 EFO/EET 官方成立日與摘要績效，因此這是日線路徑與組合未見的半獨立驗證，不是完全盲測。<a href="https://www.proshares.com/our-etfs/leveraged-and-inverse/efo">EFO</a> 與 <a href="https://www.proshares.com/our-etfs/leveraged-and-inverse/eet">EET</a> 追求每日 2 倍，不保證長期兩倍。協議 SHA-256：<code>{html.escape(audit["protocol"]["sha256"])}</code><br>外部面板 SHA-256：<code>{html.escape(audit["snapshot"]["panel_sha256"])}</code></p></section>
...
def build_diversifier_strength_report(destination: str | Path, audit: dict[str, Any]) -> Path:
    ...
    css = _late_research_css()
    decision = (
        "所有設計、外部與資料入口全過；仍只能建立隔離 Paper，不能直接參考交易。"
        if paper_ok
        else "資料與時序 13/13 全過，但經濟門檻只有 45/154、外部只有 7/42、統計 0/27。依凍結協議拒絕建立 v20 Paper，也不顯示可交易配置。"
    )
    external = audit["datasets"]
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v20 分散器相對強弱驗證</title><style>{css}</style></head><body><main>
    ...
    <section class="verdict"><article class="card"><span>已見設計門檻</span><strong class={"pass" if audit["design_economic_passed_gate_count"] == audit["design_economic_required_gate_count"] else "fail"}>{audit["design_economic_passed_gate_count"]} / {audit["design_economic_required_gate_count"]}</strong><small>8 市場 × 14 道</small></article><article class="card"><span>新外部門檻</span><strong class={"pass" if audit["external_economic_passed_gate_count"] == audit["external_economic_required_gate_count"] else "fail"}>{audit["external_economic_passed_gate_count"]} / {audit["external_economic_required_gate_count"]}</strong><small>日本、中國大型股、巴西</small></article><article class="card"><span>資料與時序</span><strong class={"pass" if audit["data_passed_gate_count"] == audit["data_required_gate_count"] else "fail"}>{audit["data_passed_gate_count"]} / {audit["data_required_gate_count"]}</strong><small>凍結、產品、日線、成交與權重</small></article><article class="card"><span>統計門檻</span><strong class={"pass" if audit["statistical_passed_gate_count"] == audit["statistical_required_gate_count"] else "fail"}>{audit["statistical_passed_gate_count"]} / {audit["statistical_required_gate_count"]}</strong><small>外部三基準 × NW／PSR／DSR</small></article></section>
    <div class="warning {"success" if paper_ok else ""}"><b>Paper 決定：{"只准建立隔離 Paper。" if paper_ok else "不建立。"}</b><br>{decision}</div>
    <section class="panel"><h2>新手先看：輪替聽起來聰明，但沒有穩定增加價值</h2><p>這次不是只問「有沒有賺錢」，而是同時要求勝過核心 ETF、勝過固定 50/25/25、勝過同規則但不槓桿的組合，還要在回撤、成本、前後半期與滾動五年都站得住腳。結果 11 個市場的 CAGR 全部低於固定 v18 配置；多數市場的最大回撤也比核心 ETF 深。</p></section>
    <section class="panel"><h2>11 市場完整結果</h2><div class="table-wrap"><table><thead><tr><th>市場</th><th>門檻</th><th>輪替 CAGR</th><th>核心 ETF</th><th>固定 v18</th><th>同政策不槓桿</th><th>Sharpe：輪替 / 核心</th><th>回撤：輪替 / 核心</th><th>五年勝率</th></tr></thead><tbody>{"".join(rows)}</tbody></table></div></section>
    <section class="panel"><h2>三個新外部市場說了什麼</h2><p>日本：輪替 CAGR {_pct(external["japan"]["strategy_metrics"]["cagr"], 2)}，低於 EWJ 的 {_pct(external["japan"]["benchmark_metrics"]["core"]["cagr"], 2)}，最大回撤也由 {_pct(external["japan"]["benchmark_metrics"]["core"]["max_drawdown"], 1)} 加深到 {_pct(external["japan"]["strategy_metrics"]["max_drawdown"], 1)}。中國大型股 14 道全失敗，輪替 CAGR {_pct(external["china_large_cap"]["strategy_metrics"]["cagr"], 2)}，低於 FXI 的 {_pct(external["china_large_cap"]["benchmark_metrics"]["core"]["cagr"], 2)}。巴西雖減輕部分回撤，但 CAGR 仍低於 EWZ、固定 v18 與同政策不槓桿版本，不能靠單一優點覆蓋其餘失敗。</p></section>
    <section class="panel"><h2>20 年邊界</h2><p>S&P 500、Nasdaq-100 與 Dow 30 的實際 ETF 設計期是 2006-07-31 至 2026-07-31，共 20 年；中小型為 18 年。新區域 2 倍 ETF 成立較晚，外部正式期固定為 2016-09-01 至 2026-07-31，不能誠實宣稱 20 年，也沒有用合成槓桿報酬補齊。</p></section>
    <section class="panel"><h2>v19 為何先停止</h2><p>在任何 v19 日線下載與績效計算前，產品稽核發現 VGK 與 UPV 在 2015-10-01 至 2016-08-31 的基準範圍不同。v19 因資料識別失敗直接作廢；v20 只修正外部資料契約，策略規則與成功門檻完全不變。</p></section>
    ...
    <section class="panel"><h2>資料與執行門檻</h2><div class="table-wrap"><table><thead><tr><th>檢查鍵</th><th>結果</th></tr></thead><tbody>{_research_gate_rows(audit["data_gates"])}</tbody></table></div></section>
...
def build_hybrid_leverage_core_report(destination: str | Path, audit: dict[str, Any]) -> Path:
    ...
    decision = (
        "128 道經濟門檻與十道資料門檻全過；仍只能從零開始隔離 Paper。"
        if paper_ok
        else (
            f"資料與治理 {audit['data_passed_gate_count']}/"
    ...
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v21 常駐核心＋受控槓桿驗證</title><style>{css}</style></head><body><main>
    <header><div class="eyebrow">FROZEN HYBRID LEVERAGE CORE TEST</div><h1>{"歷史入口全過，仍須前瞻觀察。" if paper_ok else "中小型股外部驗證否決候選，不進 Paper。"}</h1><p class="lead">v21 永久保留 60% 普通核心 ETF；核心連續兩個完整月站上 200 日均線時，把股票名目曝險提高到約 120%，否則降到約 60%。規則、日期、產品與 128 道門檻都在 UMDD／URTY 日線下載前凍結。</p></header>
    <section class="verdict"><article class="card"><span>20 年／已見設計</span><strong class={"pass" if audit["design_economic_passed_gate_count"] == audit["design_economic_required_gate_count"] else "fail"}>{audit["design_economic_passed_gate_count"]} / {audit["design_economic_required_gate_count"]}</strong><small>6 組 × 16 道</small></article><article class="card"><span>新外部門檻</span><strong class={"pass" if audit["external_economic_passed_gate_count"] == audit["external_economic_required_gate_count"] else "fail"}>{audit["external_economic_passed_gate_count"]} / {audit["external_economic_required_gate_count"]}</strong><small>中型股、小型股各 16 道</small></article><article class="card"><span>資料與治理</span><strong class={"pass" if audit["data_passed_gate_count"] == audit["data_required_gate_count"] else "fail"}>{audit["data_passed_gate_count"]} / {audit["data_required_gate_count"]}</strong><small>先凍結、產品、日線、成交、權重</small></article><article class="card"><span>Paper 決定</span><strong class={"pass" if paper_ok else "fail"}>{"可建立" if paper_ok else "不建立"}</strong><small>失敗候選不提供可照抄訊號</small></article></section>
    <div class="warning {"success" if paper_ok else ""}"><b>結論：{"只准隔離 Paper。" if paper_ok else "不要照這個候選交易。"}</b><br>{decision}</div>
    <section class="panel"><h2>新手先看：這個開關在做什麼</h2><p>把普通 ETF 的完整股票風險想成 100 格。v21 在市場趨勢確認向上時約開到 120 格；確認轉弱時降到 60 格，但不完全離開市場。這原本想同時避免 v14「退太多、錯過反彈」與 v15「留太多、回撤過深」；真正的測試不是它有沒有賺錢，而是能否同時勝過普通 ETF、永久 120 格與永久 60 格三種公平對照。</p></section>
    <section class="panel"><h2>八組完整結果</h2><div class="table-wrap"><table><thead><tr><th>市場</th><th>門檻</th><th>v21 CAGR</th><th>普通 ETF</th><th>固定 120% 曝險</th><th>固定 60% 曝險</th><th>Sharpe：v21 / 普通</th><th>回撤：v21 / 普通</th><th>五年勝率</th></tr></thead><tbody>{"".join(rows)}</tbody></table></div></section>
    <section class="panel"><h2>外部路徑為何否決</h2><p>S&P MidCap 400 的 v21 CAGR 為 {_pct(mid["strategy_metrics"]["cagr"], 2)}，低於 IJH 的 {_pct(mid["benchmark_metrics"]["core"]["cagr"], 2)}；最大回撤 {_pct(mid["strategy_metrics"]["max_drawdown"], 1)}，也深於 IJH 的 {_pct(mid["benchmark_metrics"]["core"]["max_drawdown"], 1)}。Russell 2000 的 v21 CAGR {_pct(small["strategy_metrics"]["cagr"], 2)}，低於 IWM 的 {_pct(small["benchmark_metrics"]["core"]["cagr"], 2)}；最大回撤 {_pct(small["strategy_metrics"]["max_drawdown"], 1)}，深於 IWM 的 {_pct(small["benchmark_metrics"]["core"]["max_drawdown"], 1)}。兩組各只通過 2/16，不能靠 Nasdaq 成功或 pooled 平均抵銷。</p></section>
    <section class="panel"><h2>20 年證據邊界</h2><p>SPY／SSO、QQQ／QLD、DIA／DDM 的實際 2 倍產品設計診斷固定為 2006-07-31 至 2026-07-31，是真實產品 20 年路徑；3 倍產品成立較晚，正式期只能固定為 2011-07-29 至 2026-07-31，共 15 年。沒有用理論三倍報酬補出不存在的 20 年產品史。大型股六組都已在先前研究看過，只能作設計診斷；新中小型股日線在 v21 凍結後才下載，但產品頁摘要績效先前已見，因此屬半獨立日線驗證，不是完全盲測。</p></section>
    <section class="panel"><h2>為何每日 3 倍不是長期三倍</h2><p><a href="https://www.proshares.com/globalassets/proshares/prospectuses/umdd_summary_prospectus.pdf">UMDD</a> 與 <a href="https://www.proshares.com/globalassets/proshares/prospectuses/urty_summary_prospectus.pdf">URTY</a> 的官方文件都把目標限定為單日。每日重設、波動、複利、費用與追蹤誤差會讓多年結果顯著偏離指數長期報酬的三倍。本研究直接使用實際 ETF 調整後 OHLCV，而不是合成三倍價格。</p></section>
    ...
    <section class="panel"><h2>資料、月末、成交與權重門檻</h2><div class="table-wrap"><table><thead><tr><th>檢查鍵</th><th>結果</th></tr></thead><tbody>{_research_gate_rows(audit["data_gates"])}</tbody></table></div></section>
...
def build_sector_capital_efficiency_report(destination: str | Path, audit: dict[str, Any]) -> Path:
    ...
    decision = (
        "所有凍結的多數一致性、合併路徑、資料與治理門檻均通過；仍只准從現金開始候選 Paper。"
        if paper_ok
        else (
            f"產業個別門檻 {audit['individual_passed_gate_count']}/"
            f"{audit['individual_required_gate_count']}；整體經濟門檻 "
            f"{audit['economic_passed_gate_count']}/"
            f"{audit['economic_required_gate_count']}；資料 "
            f"{audit['data_passed_gate_count']}/{audit['data_required_gate_count']}。"
            "任一凍結入口失敗就不建立 Paper。"
        )
    )
    css = _late_research_css()
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v22 美國產業股債金驗證</title><style>{css}</style></head><body><main>
    ...
    <section class="verdict"><article class="card"><span>個別產業門檻</span><strong class={"pass" if audit["individual_passed_gate_count"] >= 45 else "fail"}>{audit["individual_passed_gate_count"]} / {audit["individual_required_gate_count"]}</strong><small>需同時滿足各門檻多數與無嚴重失敗</small></article><article class="card"><span>九產業等權</span><strong class={"pass" if pooled_passed == 9 else "fail"}>{pooled_passed} / 9</strong><small>不能由單一科技或防禦產業撐起</small></article><article class="card"><span>資料與治理</span><strong class={"pass" if audit["data_passed_gate_count"] == audit["data_required_gate_count"] else "fail"}>{audit["data_passed_gate_count"]} / {audit["data_required_gate_count"]}</strong><small>先凍結、同指數、單次下載、極端值核對</small></article><article class="card"><span>Paper 決定</span><strong class={"pass" if paper_ok else "fail"}>{"可建立" if paper_ok else "不建立"}</strong><small>trade_ready 永遠仍為 false</small></article></section>
    <div class="warning {"success" if paper_ok else ""}"><b>結論：{"只准候選 Paper。" if paper_ok else "不要把這套比例當成下單訊號。"}</b><br>{decision}</div>
    <section class="panel"><h2>新手先看：為什麼是 50／25／25</h2><p>2 倍 ETF 的 50% 物理權重約提供 100 格股票曝險，再加 25 格中期美債與 25 格黃金。這不是預測漲跌，而是用產品內槓桿騰出資金放入兩種分散器；代價是每日重設、費用與極端行情會放大誤差。每月只做一次回到目標比例的再平衡。</p></section>
    <section class="panel"><h2>九個尚未看過的產業路徑</h2><div class="table-wrap"><table><thead><tr><th>產業</th><th>門檻</th><th>v22 CAGR</th><th>普通 ETF</th><th>Sharpe：v22 / ETF</th><th>回撤：v22 / ETF</th><th>五年勝率</th></tr></thead><tbody>{"".join(rows)}</tbody></table></div></section>
    <section class="panel"><h2>九產業等權結果</h2><p>v22 CAGR {_pct(strategy["cagr"], 2)}，普通產業 ETF 等權 {_pct(core["cagr"], 2)}；Sharpe {_num(strategy["sharpe"])} 對 {_num(core["sharpe"])}；最大回撤 {_pct(strategy["max_drawdown"], 1)} 對 {_pct(core["max_drawdown"], 1)}。合併路徑通過 {pooled_passed}/9 道，不能用個別產業的亮點抵銷整體失敗。</p></section>
    <section class="panel"><h2>資料異常如何處理</h2><p>唯一超過 35% 的旗標是 DIG 在 2008-10-13 的市場收盤報酬 +36.29%。ProShares 官方 NAV 同日為 +37.61%，普通 IYE 同日為 +24.38%，證明是金融危機反彈，不是漏調整拆股。原始值完整保留，原機械失敗 ZIP 也未刪除，沒有重抓或放寬門檻。</p></section>
...
def build_managed_futures_capital_efficiency_report(
    ...
    decision = (
        ...
        if paper_ok
            ...
            "20 年段只有很小的完整期報酬優勢，後十年、成本與滾動穩定性失敗；"
            "另一個管理人 FMF 也沒有保留跑贏 SPY 的方向，因此不建立 Paper。"
        )
    )
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v23 美國股＋管理期貨驗證</title><style>{css}</style></head><body><main>
    <header><div class="eyebrow">FROZEN 20-YEAR CAPITAL-EFFICIENCY TEST</div><h1>{"歷史入口通過，仍只准候選 Paper。" if paper_ok else "風險路徑改善，但沒有穩健跑贏 SPY。"}</h1><p class="lead">v23 唯一候選是每月 50% SSO／50% KMLM：用半數實體資金的每日 2 倍 S&P 500 約保留 100% 股票曝險，另一半放進商品、貨幣與全球公債的多空趨勢策略。沒有搜尋其他比例或擇時參數。</p></header>
    ...
    <div class="warning {"success" if paper_ok else ""}"><b>結論：{"只准候選 Paper。" if paper_ok else "不要照 50／50 下單。"}</b><br>{decision}</div>
    <section class="panel"><h2>新手先看：有改善，不等於跑贏</h2><p>20 年候選 CAGR {_pct(long_strategy["cagr"], 2)}，SPY {_pct(long_spy["cagr"], 2)}，只多 {_pct(long_strategy["cagr"] - long_spy["cagr"], 2)}／年，未達事前要求的 0.25%。它把最大回撤由 {_pct(long_spy["max_drawdown"], 1)} 改善到 {_pct(long_strategy["max_drawdown"], 1)}，Sharpe 也略升；但 50 bps 成本後反而落後 SPY {_pct(abs(long_data["cost_50bps"]["cagr_difference"]), 2)}／年。這是一個較平滑的歷史路徑，不是已證明的超額報酬。</p></section>
    <section class="panel"><h2>三段固定驗證</h2><div class="table-wrap"><table><thead><tr><th>證據段</th><th>候選 CAGR</th><th>SPY CAGR</th><th>Sharpe：候選 / SPY</th><th>回撤：候選 / SPY</th><th>五年勝率</th></tr></thead><tbody>{period_rows}</tbody></table></div></section>
    ...
    <section class="panel"><h2>產品映射不是免費的</h2><p>KMLM 實際月報酬與 KFA 指數相關 {_num(tracking["monthly_return_correlation"], 3)}，方向一致；但 2021-01 至 2026-06 的指數 CAGR {_pct(tracking["index_cagr"], 2)}、ETF {_pct(tracking["fund_cagr"], 2)}，年化幾何追蹤差 {_pct(tracking["annualized_geometric_tracking_gap"], 2)}，超過凍結的 2.00%。官方 FAQ 說一般拖累約為 0.90% 費用加約 0.15% 交易費，也另揭露 2022 年現金管理造成額外拖累；因此歷史指數代理不能直接當成投資人可得報酬。</p></section>
    <section class="panel"><h2>統計確認</h2><p>20 年候選相對 SPY 的主動月報酬 Newey-West t = {_num(long_stats["active_return_newey_west"]["t_stat"])}，PSR = {_pct(psr["probability"], 2)}；計入全專案 {audit["global_search_trials"]:,} 次搜尋後 DSR = {_pct(dsr["probability"], 4)}。這些只作診斷，不能抵銷經濟入口失敗。</p></section>
    ...
    <section class="panel"><h2>資料與證據邊界</h2><p>20 年段不是 20 年實際 KMLM：股票袖套是實際 SSO，管理期貨袖套是 KraneShares 官方月表中 2005 起同一 EV 方法的 KFA MLM Index，再扣固定年化 1.05%。正式期恰為 240 個月；原始 24 頁官方簡報、抽取 CSV、KMLM／FMF 單次下載快照與雜湊均保留。凍結前已看過指數月表與產品摘要，所以整體不能稱完全盲測。</p></section>
    ...
    <p class="footer">歷史回測、研究與教育用途，不構成投資建議。SSO 是每日槓桿 ETF；KMLM／FMF 使用多空期貨。全部都可能快速虧損，且指數報酬不可直接投資。未通過凍結入口、前瞻 Paper 與 readiness 前，不提供可照抄交易配置。</p>
    </main></body></html>"""
    destination.write_text(content, encoding="utf-8")
    return destination
...
def build_quality_momentum_factor_report(destination: str | Path, audit: dict[str, Any]) -> Path:
    ...
    css = _late_research_css()
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v24 美國品質＋動量因子驗證</title><style>{css}</style></head><body><main>
    <header><div class="eyebrow">FROZEN QUALITY + MOMENTUM FACTOR TEST</div><h1>{"三層入口通過，仍只准候選 Paper。" if paper_ok else "教科書因子成立，但可買 ETF 沒有穩健複製。"}</h1><p class="lead">v24 只測每月固定 50% QUAL／50% MTUM。沒有挑比例、沒有 VIX、沒有槓桿，也沒有停利停損；先把品質與動量袖套本身，和產品包裝、出場規則分開歸因。</p></header>
    ...
    <div class="warning {"success" if paper_ok else ""}"><b>結論：{"只准隔離 Paper。" if paper_ok else "不要照 QUAL／MTUM 50/50 下單。"}</b><br>{"三層凍結入口已通過，但仍需 252 個新增交易日。" if paper_ok else "20 年學術排序表現漂亮；真正可買的兩組產品卻在風險、時間穩定性或完整期報酬失敗。這是因子概念與可投資產品之間的映射失敗。"}</div>
    <section class="panel"><h2>新手先看：論文中的因子，不等於 ETF 投資人拿得到</h2><p>French 20 年代理 CAGR {_pct(academic_strategy["cagr"], 2)}、市場 {_pct(academic_market["cagr"], 2)}，Sharpe {_num(academic_strategy["sharpe"])} 對 {_num(academic_market["sharpe"])}，最大回撤 {_pct(academic_strategy["max_drawdown"], 1)} 對 {_pct(academic_market["max_drawdown"], 1)}，10/10 全過。更早 1964–2006 也以 {_pct(older["strategy_metrics"]["cagr"], 2)} 對 {_pct(older["benchmark_metrics"]["MARKET"]["cagr"], 2)} 保留方向。但這些是學術排序投組，不是 QUAL／MTUM 的歷史淨值。</p></section>
    <section class="panel"><h2>實際 iShares 為什麼只過 5/10</h2><p>2013–2026 候選 CAGR {_pct(ishares_strategy["cagr"], 2)}，高於 SPY {_pct(ishares_spy["cagr"], 2)}；50 bps 成本後仍領先。但 Sharpe {_num(ishares_strategy["sharpe"])} 略低於 SPY {_num(ishares_spy["sharpe"])}，最大回撤 {_pct(ishares_strategy["max_drawdown"], 1)} 比 SPY {_pct(ishares_spy["max_drawdown"], 1)} 更深。前半領先 {_pct(ishares["fixed_halves_vs_market"]["first"]["cagr_difference"], 2)}，後半轉為落後 {_pct(abs(ishares["fixed_halves_vs_market"]["second"]["cagr_difference"]), 2)}；97 個五年窗勝率 {_pct(ishares_rolling["cagr_win_fraction"], 1)}、中位差 {_pct(ishares_rolling["median_cagr_difference"], 2)}。</p></section>
    <section class="panel"><h2>不同產品沒有保留方向</h2><p>2007–2026 的 50% SPHQ／50% PDP CAGR {_pct(invesco_strategy["cagr"], 2)}，低於 SPY {_pct(invesco_spy["cagr"], 2)}；Sharpe、最大回撤、Calmar、成本、兩半期與五年滾動七道全部失敗。173 個五年窗勝率只有 {_pct(invesco_rolling["cagr_win_fraction"], 1)}，中位 CAGR 差 {_pct(invesco_rolling["median_cagr_difference"], 2)}。這不是完全相同指數的複製，但足以否決「只要買品質＋動量 ETF 就能跨產品穩健跑贏」的廣泛說法。</p></section>
    ...
    <section class="panel"><h2>資料與證據邊界</h2><p>正式 20 年恰為 {audit["academic_integrity"]["formal_months"]} 個月，使用 French `BIG HiOP`、`BIG HiPRIOR` 與 `Mkt-RF + RF`，並扣固定年化 0.15% 代理費用。MTUM 實際基準為拼接指數且後續方法有變；SPHQ／PDP 又是不同供應商與不同定義。凍結前看過產品摘要，因此不能稱完全盲測；所有聯合路徑都在協議凍結後才計算。</p></section>
...
def build_growth_gold_diversification_report(
    ...
    path_sections = "".join(
        f"""<section class="panel"><h2>{html.escape(label.replace("_", " ").title())} 實際產品路徑</h2><p>候選 CAGR {_pct(row["strategy_metrics"]["cagr"], 2)}、SPY {_pct(row["benchmark_metrics"]["SPY"]["cagr"], 2)}、100% {html.escape(row["implementation"]["growth"])} {_pct(row["benchmark_metrics"]["growth"]["cagr"], 2)}、相同成長曝險加 SHY {_pct(row["benchmark_metrics"]["matched_80_growth_20_SHY"]["cagr"], 2)}。候選 Sharpe {_num(row["strategy_metrics"]["sharpe"])}、最大回撤 {_pct(row["strategy_metrics"]["max_drawdown"], 1)}；相對 SPY 的 181 個五年窗勝率 {_pct(row["rolling_five_year_vs_SPY"]["summary"]["cagr_win_fraction"], 1)}，相對純成長只有 {_pct(row["rolling_five_year_vs_growth"]["summary"]["cagr_win_fraction"], 1)}。診斷沒有加入凍結門檻或改變通過數。</p><div class="table-wrap"><table><thead><tr><th>檢查鍵</th><th>結果</th></tr></thead><tbody>{_research_gate_rows(row["entry_gates"])}</tbody></table></div></section>"""
        for label, row in audit["paths"].items()
    )
    css = _late_research_css()
    content = f"""<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>v25 美國大型成長＋黃金分散驗證</title><style>{css}</style></head><body><main>
    <header><div class="eyebrow">FROZEN 20-YEAR GROWTH + GOLD PRODUCT TEST</div><h1>{"歷史產品入口通過，只啟動隔離 Paper。" if paper_ok else "實際產品入口未通過，不建立 Paper。"}</h1><p class="lead">v25 每月固定 80% 美國大型成長 ETF、20% 實物黃金 ETF。沒有預測、槓桿、停損或權重搜尋；Vanguard、iShares、State Street 三條實際產品路徑全部保留。</p></header>
    ...
    <div class="warning {"success" if paper_ok else ""}"><b>現在怎麼做：{"只建立 Paper，不用實金照抄。" if paper_ok else "保持研究，不下單。"}</b><br>{"20 年產品入口已通過；但相對 SPY 的統計確認仍弱，且最差五年窗仍會落後。Paper 必須再累積至少 252 個真正新增交易日與 6 次初始建倉後的月度再平衡。" if paper_ok else "任何一條產品或彙總入口失敗，都不能顯示 80/20 為今天配置。"}</div>
    <section class="panel"><h2>新手先看：這個策略靠什麼</h2><p>80 格大型成長股提供企業成長曝險，20 格黃金提供不同報酬來源；每月只把漂移比例拉回 80/20。黃金不是現金、沒有利息，也可能和股票同跌。這套規則的 20 年彙總 CAGR {_pct(strategy["cagr"], 2)}，SPY {_pct(spy["cagr"], 2)}；最大回撤 {_pct(strategy["max_drawdown"], 1)}，SPY {_pct(spy["max_drawdown"], 1)}。這表示歷史上勝過 SPY，不表示勝過所有 ETF。</p></section>
    <section class="panel"><h2>不能省略的比較：它沒有跑贏 100% 大型成長</h2><p>三路徑純成長 ETF 彙總 CAGR {_pct(growth["cagr"], 2)}，高於 v25 的 {_pct(strategy["cagr"], 2)}；v25 每年少 {_pct(abs(stats_growth["cagr_difference"]), 2)}。換到的是 Sharpe 從 {_num(growth["sharpe"])} 提高至 {_num(strategy["sharpe"])}，最大回撤從 {_pct(growth["max_drawdown"], 1)} 改善至 {_pct(strategy["max_drawdown"], 1)}，約少跌 {_pct(stats_growth["drawdown_improvement"], 1)}。181 個五年窗只有 {_pct(rolling_growth["cagr_win_fraction"], 1)} 勝過純成長，最差落後 {_pct(abs(rolling_growth["worst_cagr_difference"]), 2)}。這是風險取捨，不是免費超額報酬。</p></section>
    <section class="panel"><h2>公平比較：不是只靠少持股票</h2><p>相同 80% 成長股、20% SHY 控制 CAGR {_pct(matched["cagr"], 2)}、Sharpe {_num(matched["sharpe"])}、最大回撤 {_pct(matched["max_drawdown"], 1)}。v25 分別為 {_pct(strategy["cagr"], 2)}、{_num(strategy["sharpe"])}、{_pct(strategy["max_drawdown"], 1)}；相對控制的 NW t = {_num(stats_matched["active_return_newey_west"]["t_stat"])}。這代表歷史差異主要來自黃金袖套，而不只是把 20% 放低波動資產。</p></section>
    ...
    <section class="panel"><h2>回撤多久才回來，比跌多少同樣重要</h2><p>v25 自己最久曾連續 {portfolio_underwater["max_underwater_months"]} 個月低於先前淨值高點：高點 {portfolio_underwater["longest_episode"]["peak"]}、谷底 {portfolio_underwater["longest_episode"]["trough"]}、至 {portfolio_underwater["longest_episode"]["recovery"]} 才復原。相對 SPY 的累積財富曾 {relative_spy["max_underwater_months"]} 個月沒有回到先前相對高點；相對純成長 ETF 的最長相對水下期為 {relative_growth["max_underwater_months"]} 個月，期末仍未復原，離先前相對高點仍低 {_pct(abs(relative_growth["current_drawdown"]), 1)}。這不代表每個月都輸，而是長期持有者可能多年感覺選錯策略。</p></section>
    <section class="panel"><h2>區塊重抽樣：漂亮全期仍可能換個順序就落後</h2><p>將候選與基準的月報酬配對，以 12 個月區塊循環重抽 10,000 次後，v25 CAGR 勝 SPY 的比例為 {_pct(bootstrap_spy["probability_cagr_above"], 1)}，報酬與回撤同時不差於 SPY 只有 {_pct(bootstrap_spy["probability_cagr_above_and_drawdown_not_worse"], 1)}；CAGR 差的第 5 百分位是 {_pct(bootstrap_spy["cagr_difference_percentiles"]["p05"], 2)}。對純成長的 CAGR 勝出比例只有 {_pct(bootstrap_growth["probability_cagr_above"], 1)}。對相同 80/20 SHY 控制，CAGR 勝出 {_pct(bootstrap_matched["probability_cagr_above"], 1)}，但報酬與回撤同時勝出只有 {_pct(bootstrap_matched["probability_cagr_above_and_drawdown_not_worse"], 1)}。6、12、24 個月區塊均有保存；這只是同一歷史樣本的順序敏感度，不處理選擇偏誤、制度改變，也不是未來勝率。</p></section>
    ...
    <section class="panel"><h2>第一筆成交前已凍結的前瞻升級合約</h2><p>首次 80/20 建倉不算完成再平衡。至少 252 個全新交易日與 6 次後續月度再平衡後，候選扣成本年化報酬仍須分別勝 SPY 與 80% VUG／20% SHY 至少 0.10 個百分點；固定前後兩半都要達標、最大回撤不得更深，而且兩組每日主動報酬的 Newey–West t 值都須至少 1.96。任一項失敗就維持 Paper-only，不能用一年期末剛好領先升級。</p></section>
    {path_sections}
    <section class="panel"><h2>三路徑彙總門檻</h2><p>以下仍是凍結前定義的 10 道入口；後加的純成長比較、回撤復原與進場時點診斷只增加透明度，不參與通過數。</p><div class="table-wrap"><table><thead><tr><th>檢查鍵</th><th>結果</th></tr></thead><tbody>{_research_gate_rows(pooled["entry_gates"])}</tbody></table></div></section>
    <section class="panel"><h2>資料與不可回改收據</h2><p>三條路徑皆為 2006-08 至 2026-07、恰 240 個月；VUG 2026 年 6:1 拆股的調整後行情稽核已通過。協議 SHA-256：<code>{html.escape(audit["protocol"]["sha256"])}</code><br>產品映射 SHA-256：<code>{html.escape(audit["protocol"]["product_mapping_sha256"])}</code><br>資料門檻：{audit["data_passed_gate_count"]} / {audit["data_required_gate_count"]}。</p></section>
    <p class="footer">歷史回測、研究與教育用途，不構成投資建議。大型成長股可能長期估值收縮；黃金不生息、可能多年落後，分散也不保證獲利。Paper 尚未累積前瞻證據，現在不提供實金下單指令。</p>
    </main></body></html>"""
    destination.write_text(content, encoding="utf-8")
    return destination
