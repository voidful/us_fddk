...
from usfddk.short_term_volume_breakout_diagnostic import (
    VALIDATION_PATH,
    audit_volume_breakout_diagnostic,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Audit a frozen volume-confirmed breakout diagnostic without trade authorization."
    )
    parser.add_argument("--repository-root", type=Path, default=Path.cwd())
    parser.add_argument("--snapshot", type=Path, required=True)
    args = parser.parse_args()
    result = audit_volume_breakout_diagnostic(
