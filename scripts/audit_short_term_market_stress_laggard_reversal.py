...
from usfddk.short_term_market_stress_laggard_reversal import (
    VALIDATION_PATH,
    audit_market_stress_laggard_reversal,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Audit a frozen market-stress laggard reversal diagnostic "
            "without trade authorization."
        )
    )
    parser.add_argument("--repository-root", type=Path, default=Path.cwd())
    parser.add_argument("--snapshot", type=Path, required=True)
    args = parser.parse_args()
    result = audit_market_stress_laggard_reversal(
