...
PROTOCOL = ROOT / "docs/SHORT_TERM_US_VOLUME_BREAKOUT_20Y_PROTOCOL.md"
OUTPUT_DEFAULT = ROOT / "artifacts/short_term_us_volume_breakout_20y_diagnostic.json"
REPORT_DEFAULT = ROOT / "docs/SHORT_TERM_US_VOLUME_BREAKOUT_20Y_REPORT.md"
...
def _build_candidates(
    ...
    for index in np.flatnonzero(weekly_mask):
        ...
        if not np.all(np.isfinite(spy_history)) or not (
            spy_history[-1] > spy_history.mean()
        ):
        ...
        volume_history = stock_volume[
            index - VOLUME_LOOKBACK + 1 : index + 1
        ]
        prior_close_history = stock_close[index - MOMENTUM_SESSIONS]
        prior_breakout_history = stock_close[index - BREAKOUT_LOOKBACK : index]
        finite = (
            ...
            & np.isfinite(prior_close_history)
            & np.all(np.isfinite(prior_breakout_history), axis=0)
        )
        close_today = stock_close[index]
        momentum = close_today / prior_close_history - 1.0
        ...
        prior_high = np.max(prior_breakout_history, axis=0)
        base = (
            finite
        ...
        breakout = base & (close_today >= prior_high) & (
            stock_volume[index, :stock_count] >= VOLUME_MULTIPLE * median_volume
        )
        ...
        if not np.all(forward_complete):
            ...
            continue
        entry_day = sessions[entry_index].date()
        signal_day = sessions[index].date()
...
def _accept_nonoverlap(
    ...
    for entry_day in sorted(by_entry):
        ...
        last_exit = exit_day
    return accepted, {
        "accepted_events": len(accepted) // TOP_K,
...
def _build_report(payload: dict[str, Any]) -> str:
    ...
    return dedent(
        ...
        版本：v1｜狀態：`{payload['decision']['strategy_status']}`｜用途：固定突破機制診斷，並非買入名單、Paper 指令或投資建議。
        ...
        - 候選事件 {payload['schedule']['candidate_events']} 宗，非重疊後接受 {payload['schedule']['accepted_events']} 宗（{ten['signal_count']} 個股票訊號）。
        - 10／25／50 bps 策略 CAGR 為 {_pct(ten['portfolio']['cagr'])}／{_pct(twenty_five['portfolio']['cagr'])}／{_pct(fifty['portfolio']['cagr'])}；QQQ 對應 {_pct(ten['QQQ']['cagr'])}／{_pct(twenty_five['QQQ']['cagr'])}／{_pct(fifty['QQQ']['cagr'])}。
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="建立 20 年成交量突破診斷；不產生交易指令"
    )
    ...
    payload: dict[str, Any] = {
        "schema_version": 1,
        "status": "post_hoc_us_volume_breakout_20y_diagnostic",
        "protocol": {
        ...
        "decision": {
            ...
            "diagnostic_status": "volume_breakout_cost_and_survivorship_diagnostic",
            "gate_summary": {
            ...
            "reason": "The snapshot is a current-watchlist survivor cohort and the fixed breakout test is not a promotion authorization.",
        },
    }
