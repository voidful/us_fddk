...
from usfddk.short_term_volume_breakout_top10_nonoverlap import (
    VALIDATION_PATH,
    audit_volume_breakout_top10_nonoverlap,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Audit the frozen Round62 Top-10 concentration diagnostic."
    )
    parser.add_argument("--repository-root", type=Path, default=Path.cwd())
    parser.add_argument("--snapshot", type=Path, required=True)
    args = parser.parse_args()
    result = audit_volume_breakout_top10_nonoverlap(
