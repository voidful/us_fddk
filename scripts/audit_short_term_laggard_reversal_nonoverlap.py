...
from usfddk.short_term_laggard_reversal_nonoverlap import (
    VALIDATION_PATH,
    audit_laggard_reversal_nonoverlap,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Audit the frozen Round60 non-overlap capital-accounting diagnostic."
    )
    parser.add_argument("--repository-root", type=Path, default=Path.cwd())
    parser.add_argument("--snapshot", type=Path, required=True)
    args = parser.parse_args()
    result = audit_laggard_reversal_nonoverlap(
